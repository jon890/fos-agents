...
plan 진행 cycle: `skills/planning` 구조로 사용자와 대화하며 brief/결정/phase를 확정 → 확정된 내용을 먼저 docs/ADR에 반영 → docs 결정을 task 파일로 옮김 → 구현 phase는 `skills/plan-and-build`가 사용 가능하면 그 흐름으로 수행하고, 없으면 동일한 phase 문서 기반의 별도 구현 세션/Claude 비대화형 실행으로 수행 → Codex가 산출물과 검증 결과를 직접 review → worktree가 깨끗하고 범위가 명확할 때만 commit/push. 세션 격리 원칙 — planning은 대화형 합의와 문서/계획 고정까지, 실 phase 실행은 별도 구현 세션.
...
- task 파일 작성은 사용자와 Codex 메인 세션에서 planning 결정이 확정된 뒤 subagent에 위임할 수 있다. 이때 subagent의 역할은 docs에 기록된 결정을 `tasks/plan.../index.json`과 `phase-NN.md`로 옮기는 것에 한정한다. 구현 코드 수정, phase 실행, commit/push는 맡기지 않는다.
- subagent가 작성한 task 파일은 Codex 메인 세션이 `index.json` 스키마, phase 독립성, 실행 가능한 성공 기준, PHASE_BLOCKED/PHASE_FAILED 조건, `common-pitfalls.md` self-check를 직접 검토한 뒤에만 확정한다.
...
- 긴 계획 파일 작성, 다중 phase task 생성, 구현 phase 실행은 메인 세션 대화를 오래 블로킹하지 않는 것을 기본값으로 한다. 사용자와의 결정 합의는 메인 세션에서 하되, 합의가 끝난 뒤 남은 문서화·task 파일 작성·phase 실행이 길어질 것 같으면 `systemd-run --user`, `plan-and-build`, Claude 비대화형 실행, 또는 동등한 백그라운드 작업으로 넘기고 완료 노티를 받는다.
...
- 최종 이력서, 지원 패키지, 외부 제출, 공개 발행은 사용자 검토 gate를 통과한 뒤에만 진행한다.
