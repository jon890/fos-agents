...
- task 파일 생성을 시작할 때는 HUD를 `task materializing` 또는 `task files running`으로 갱신하고, 생성/검토/commit/push가 끝나면 `task files completed`로 갱신한다.
  OpenClaw career 세션에서는 `refresh-hud` skill 또는 `.openclaw/workspace-career/scripts/task-hud/update_event.ts`를 사용해 `discord-career-main` HUD를 갱신한다.
...
**현재 표준 진입점은 native skill 6개다.** `claude -p "/<skill-name> <args>"` 직접 호출만 사용한다.
