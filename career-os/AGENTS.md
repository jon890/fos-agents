...
- 같은 하네스는 `<workspace>/.env`에 `TASK_HUD_SESSION_ID`, `TASK_HUD_TARGET`, `OPENCLAW_WORKSPACE_ROOT`가 있으면 phase 시작/완료/실패/보류 이벤트를 HUD에도 반영한다.
  HUD 갱신 실패는 phase 실패로 보지 않지만, 메인 세션은 완료 후 HUD가 stale이면 `refresh-hud`로 실측 갱신한다.
...
- 작업 완료 보고 전에는 반드시 `git status --short`를 다시 확인한다.
  보고에는 worktree 상태를 `clean`, `unrelated dirty만 남음`, `의도적으로 남긴 파일 있음`, `정리 보류`처럼 분류해 적는다.
...
- 별도 git worktree를 만든 background 작업은 종료 전에 해당 worktree에서 `git status --short`가 비어 있는지 확인한다.
  clean이면 `git worktree remove <path>`까지 수행한 뒤 완료로 본다.
  dirty이면 worktree를 제거하지 않고 남은 변경, worktree 경로, 다음 조치를 보고한다.
- subagent나 백그라운드 작업자에게 일을 맡길 때도 위 worktree 정리 계약을 프롬프트에 포함한다.
