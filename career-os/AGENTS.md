...
plan 진행 cycle: `skills/planning` 구조로 사용자와 대화하며 brief/결정/phase를 확정 → 합의된 task 파일 작성·commit·push → 구현 phase는 `skills/plan-and-build`가 사용 가능하면 그 흐름으로 수행하고, 없으면 동일한 phase 문서 기반의 별도 구현 세션/Claude 비대화형 실행으로 수행 + Codex 검토/검증. 세션 격리 원칙 — planning은 대화형 합의와 task 생성까지, 실 phase 실행은 별도 구현 세션.

중요 운영 원칙:

- 사용자가 계획, 로드맵, 단계 정리, 구현 방향 논의를 요청하면 먼저 OpenClaw `skills/planning/SKILL.md`를 로드하고 그 구조(목표 재정의, phase, 열린 결정, 다음 액션)로 설계한다.
...
- Claude/서브에이전트가 구현한 결과는 곧바로 신뢰하지 않는다. 메인 세션(Codex)이 `git diff`, build/test/smoke, 정책 grep, runner gate를 직접 재실행한 뒤에만 완료로 본다.
...
- worktree가 이미 dirty하면 commit/push는 보류한다. 관련 diff와 검증 결과를 보고하고, unrelated 변경을 포함해 커밋하지 않는다.
...
- 주 트랙: **Java 백엔드**
- 현재 MVP에서 Kotlin 제외
- 현재 집중 보강 축: **CJ Foodville 1차 면접 실전 답변력** — F&B/e-Commerce 도메인 언어, 운영 안정성, 메시징/트랜잭션 경계, 캐시 정합성, 레거시 공존, 경험 번역.
- 목표: 실전 면접 중심의 일일 학습 가이드. 단일 "약점 풀"보다 최근 학습 이력, 관심사, 면접 일정, 타깃 직무 맥락을 함께 반영한다.

## 지원 공고 수집 원칙

application-flow-agent와 지원 패키지 생성 흐름의 입력은 **실제 지원 가능한 개별 공고**로 제한한다.

- 채용 메인 페이지, 회사명, 블로그/뉴스 기반 추천, "이 회사가 좋아 보인다" 수준의 단서는 지원 공고로 수집하지 않는다.
- 개별 공고 URL이 없거나 현재 모집 중인지 확인되지 않은 항목은 `data/applications/ledger.jsonl`에 넣지 않는다.
- `position-recommender` 리포트에 좋은 회사/도메인 후보가 있어도, 개별 공고 URL이 없으면 agent runtime 입력으로 사용하지 않는다.
- 공고 수집 대상은 개별 JD URL, 모집 활성 여부, 역할/연차/스택, Java/Spring 백엔드 적합성, 지원 경로가 확인된 항목으로 한정한다.
- `ledger.jsonl`은 "지원 가능한 공고 원장"이다. 단순 탐색 단서나 lead 목록을 별도로 만들지 않는다.
- `actionableCandidate=true`는 개별 공고 검증과 후보자 fit 검토를 통과한 뒤에만 허용한다.
- 최종 이력서 또는 지원 패키지 산출물이 검증되기 전에는 사용자 검토 대기 상태로 넘기지 않는다.
...
- 사용자 설명과 Discord 요약에서는 불필요한 한국어/영어 혼용을 피한다. `bullet`, `target`, `gate`, `rubric` 같은 영어 용어를 쓸 때는 처음에 쉬운 한국어로 풀어쓴다. 예: "target별 이력서 bullet" 대신 "지원 회사/직무별 이력서 한 줄 경력 문장". 파일명·코드·공식 용어는 유지하되, 사용자가 이해할 설명은 한국어를 우선한다.
