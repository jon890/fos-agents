...
| [`docs/code-architecture.md`](docs/code-architecture.md) | 디렉터리 레이어·책임·외부 의존·agent skill 실행 패턴 | 코드 구조 변경 / 새 스킬 추가 |
...
- planning skill의 비대화형 CLI 호출은 기본 사용하지 않는다.
  planning skill 호출 자체도 특정 에이전트 백그라운드 작업으로 넘기지 않는다.
  planning은 사용자의 반박/결정 보류/범위 조정을 반영해야 하므로 Codex와 사용자가 메인 세션에서 같은 planning 흐름으로 대화하며 진행한다.
...
- 문서와 운영 메모는 한국어를 기본으로 작성한다.
  `TOOLS.md`도 사용자가 직접 읽고 수정할 수 있는 문서이므로, 경로·명령·env key 같은 식별자를 제외한 설명 문장은 한국어로 쓴다.
...
- 스킬이나 source adapter를 변경하면 즉시 1회 호출·수집으로 동작을 검증한다. source별 수집 수, reject diagnostics, active-only 누수, 추천 입력 품질을 그 자리에서 확인하고 문제가 있으면 바로 고친다. shadow 관찰 기간을 두지 않는다 — 변경과 검증을 즉시즉시 붙인다.
...
- 공개 공부팩은 특정 회사/포지션/지원 여부를 전제로 쓰지 않는다. 타깃 회사의 관점(중시하는 기술 깊이·운영 포인트)은 학습 방향을 잡는 데만 반영하고, 본문에는 특정 회사·포지션 이름을 쓰지 않으며 그 회사에만 적용되는 내용으로 좁히지 않는다. 같은 주제를 공부하는 누구에게나 유효한 일반 기술 문서로 작성한다.
