...
async function service() {
  ...
  await service.updateCompanyPreference("테스트 회사", {
    companyKey: "테스트 회사",
    companyName: "테스트 회사",
    tier: 3,
    disposition: "analyze",
...
async function prepare(
  target: PositionService,
  body: { schemaVersion: 2; analysisContractVersion: number; pool: PostingCandidatePool },
  now?: string,
) {
  const preparation = await target.saveCollection(body, now);
  expect(preparation.companyTierQueue.companies).toHaveLength(0);
  return target.createPositionAnalysisRun(preparation.collectionRunId, now);
}
...
describe("position service", () => {
  ...
  test("본문 변경은 changed로 분류하고 과거 version과 분석을 보존한다", async () => {
    ...
    const changed = await prepare(target, {
      schemaVersion: 2,
      analysisContractVersion: 1,
  ...
  test("부분 실패 응답에는 소스명과 실패 건수만 공개한다", async () => {
    ...
    const queue = await prepare(target, {
      schemaVersion: 2,
      analysisContractVersion: 1,
  ...
  test("partial 소스에서 보이지 않는 기존 공고 lifecycle을 유지한다", async () => {
    ...
    await prepare(target, { schemaVersion: 2, analysisContractVersion: 1, pool: missing });
...
describe("회사 tier 평가 큐", () => {
  function companyPool(
    runId: string,
    companies: string[],
    collectedAt = "2026-09-17T00:00:00.000Z",
  ): PostingCandidatePool {
    const base = pool(runId);
    base.collectedAt = collectedAt;
    base.candidates = companies.map((company, index) => ({
      ...base.candidates[0],
      id: `wanted:candidate-${index + 1}`,
      identityHash: `wanted:${index + 1}`,
      url: `https://example.com/jobs/${index + 1}`,
      company,
    }));
    base.sourceDiagnostics[0].collectedCount = companies.length;
    base.sourceDiagnostics[0].importedCount = companies.length;
    return base;
  }

  function tierResult(companyKey: string, recommendedTier: number) {
    return {
      companyKey,
      recommendedTier,
  ...
  async function collect(target: PositionService, runId: string, companies: string[], now: string) {
    return target.saveCollection(
      { schemaVersion: 2, analysisContractVersion: 1, pool: companyPool(runId, companies, now) },
      now,
    );
  }
  ...
  async function assess(
    ...
    results: Array<{ companyKey: string; tier: number }>,
    now: string,
    failures: Array<{ companyKey: string; failureCode: string }> = [],
  ) {
    return target.saveCompanyTierResults(
      companyTierRunId,
      {
        schemaVersion: 1,
        collectionRunId,
        results: results.map((entry) => tierResult(entry.companyKey, entry.tier)),
        failures,
      },
      now,
    );
  ...
  test("수동 tier와 exclude는 큐에서 빠지고 모델 평가보다 먼저 적용된다", async () => {
    const { repository, service: target } = await service();
    await target.updateCompanyPreference("제외 회사", {
      companyKey: "제외 회사",
      companyName: "제외 회사",
      tier: 2,
      disposition: "exclude",
    });
    await target.updateCompanyPreference("수동 회사", {
      companyKey: "수동 회사",
      companyName: "수동 회사",
      tier: 1,
      disposition: "analyze",
    ...
    expect(first.companyTierQueue.companies.map((entry) => entry.companyKey)).toEqual([
      "모델 회사",
    ]);
    await assess(
      target,
      first.companyTierQueue.companyTierRunId,
      "run-priority-1",
      [{ companyKey: "모델 회사", tier: 2 }],
      "2026-09-17T01:30:00.000Z",
    );
    const queue = await target.createPositionAnalysisRun(
      "run-priority-1",
      "2026-09-17T02:00:00.000Z",
    );
    const items = [...repository.snapshot().analysisRuns.get(queue.analysisRunId)!.items.values()];
    const byCompany = new Map(
      items.map((item) => [
        [...repository.snapshot().positions.values()].find(
          (position) => position.positionId === item.positionId,
        )!.posting.company,
        item,
      ]),
    );
    expect([...byCompany.keys()].sort()).toEqual(["모델 회사", "수동 회사"]);
    expect(byCompany.get("모델 회사")).toMatchObject({ companyTier: 2, companyTierSource: "model" });
    expect(byCompany.get("모델 회사")!.companyTierAssessmentId).not.toBeNull();
    expect(byCompany.get("수동 회사")).toMatchObject({
      companyTier: 1,
      companyTierSource: "manual",
      companyTierAssessmentId: null,
    });

    await target.updateCompanyPreference("모델 회사", {
      companyKey: "모델 회사",
      companyName: "모델 회사",
      tier: 3,
      disposition: "analyze",
    ...
    expect(second.companyTierQueue.companies).toHaveLength(0);
    const secondQueue = await target.createPositionAnalysisRun(
      "run-priority-2",
      "2026-09-18T02:00:00.000Z",
    );
    const secondItems = [
      ...repository.snapshot().analysisRuns.get(secondQueue.analysisRunId)!.items.values(),
    ];
    expect(
      secondItems.every(
        (item) => item.companyTierSource === "manual" && item.companyTierAssessmentId === null,
      ),
    ).toBe(true);
  });

  test("유효한 모델 평가는 큐에 다시 실리지 않고 공고 우선순위에 재사용된다", async () => {
    const { repository, service: target } = await service();
    const first = await collect(target, "run-reuse-1", ["가 회사"], "2026-09-17T01:00:00.000Z");
    await assess(
      target,
      first.companyTierQueue.companyTierRunId,
      "run-reuse-1",
    ...
    await target.createPositionAnalysisRun("run-reuse-1", "2026-09-17T02:00:00.000Z");
    const second = await collect(target, "run-reuse-2", ["가 회사"], "2026-09-18T01:00:00.000Z");
    expect(second.companyTierQueue.companies).toHaveLength(0);
    expect(second.companyTierQueue.status).toBe("completed");
    expect(second.companyTierQueue.summary).toMatchObject({
    ...
    const item = [
      ...repository.snapshot().analysisRuns.get(queue.analysisRunId)!.items.values(),
    ][0];
    expect(item).toMatchObject({ companyTier: 1, companyTierSource: "model" });
  });

  test("만료된 이전 Tier 1을 Tier 2와 3보다 먼저 재평가한다", async () => {
    const { service: target } = await service();
    const companies = ["가 회사", "나 회사", "다 회사"];
    const first = await collect(target, "run-refresh-1", companies, "2026-09-17T01:00:00.000Z");
    expect(first.companyTierQueue.companies.map((entry) => entry.assessmentStatus)).toEqual([
      "new",
      "new",
      "new",
    ]);
    await assess(
      ...
      "run-refresh-1",
      [
        { companyKey: "가 회사", tier: 3 },
        { companyKey: "나 회사", tier: 1 },
        { companyKey: "다 회사", tier: 2 },
      ],
      "2026-09-17T01:30:00.000Z",
    );
    const second = await collect(target, "run-refresh-2", companies, "2027-01-01T01:00:00.000Z");
    expect(second.companyTierQueue.companies.map((entry) => entry.companyKey)).toEqual([
      "나 회사",
      "다 회사",
      "가 회사",
    ]);
    expect(second.companyTierQueue.companies.map((entry) => entry.priorTier)).toEqual([1, 2, 3]);
    expect(
      second.companyTierQueue.companies.every((entry) => entry.assessmentStatus === "stale"),
    ).toBe(true);
    expect(second.companyTierQueue.companies[0].priorValidUntil).toBe("2026-12-16");
  });

  test("평가 실패는 실행에 남지만 기본 tier로 공고 분석을 계속한다", async () => {
    const { repository, service: target } = await service();
    const first = await collect(target, "run-fail", ["가 회사"], "2026-09-17T01:00:00.000Z");
    const response = await assess(
      ...
      [],
      "2026-09-17T01:30:00.000Z",
      [{ companyKey: "가 회사", failureCode: "research_unavailable" }],
    );
    expect(response).toMatchObject({ status: "partial", failedCount: 1, createdCount: 0 });
    const run = repository
      .snapshot()
      .companyTierRuns.get(first.companyTierQueue.companyTierRunId)!;
    expect([...run.items.values()][0]).toMatchObject({
      resultStatus: "failed",
      failureCode: "research_unavailable",
      companyTierAssessmentId: null,
      attemptCount: 1,
    });
    const queue = await target.createPositionAnalysisRun("run-fail", "2026-09-17T02:00:00.000Z");
  ...
  test("다른 실행의 유효한 임차권은 중복 평가를 막고 2시간이 지나면 회수된다", async () => {
    const { repository, service: target } = await service();
    const first = await collect(target, "run-lease-1", ["가 회사"], "2026-09-17T01:00:00.000Z");
    expect(first.companyTierQueue.companies).toHaveLength(1);

    const during = await collect(target, "run-lease-2", ["가 회사"], "2026-09-17T02:59:00.000Z");
    expect(during.companyTierQueue.companies).toHaveLength(0);
    expect(during.companyTierQueue.status).toBe("completed");
    expect(
      repository.snapshot().companyTierRuns.get(first.companyTierQueue.companyTierRunId)!.status,
    ).toBe("pending");

    const after = await collect(target, "run-lease-3", ["가 회사"], "2026-09-17T03:01:00.000Z");
    expect(after.companyTierQueue.companies.map((entry) => entry.companyKey)).toEqual(["가 회사"]);
    const expired = repository
      .snapshot()
      .companyTierRuns.get(first.companyTierQueue.companyTierRunId)!;
    expect(expired.status).toBe("partial");
    expect([...expired.items.values()][0]).toMatchObject({
      resultStatus: "failed",
  ...
  test("빈 회사 큐는 실행을 바로 완료하고 공고 분석 큐를 반환한다", async () => {
    ...
    expect(preparation.companyTierQueue).toMatchObject({
      status: "completed",
      companies: [],
      summary: { activeCompanyCount: 1, manualCount: 1, queuedCount: 0 },
    });
    expect(
      repository.snapshot().companyTierRuns.get(preparation.companyTierQueue.companyTierRunId)!
        .completedAt,
    ).toBe("2026-09-17T01:00:00.000Z");
    const queue = await target.createPositionAnalysisRun("run-empty", "2026-09-17T02:00:00.000Z");
    expect(queue.candidates).toHaveLength(1);
    expect(queue.summary).toMatchObject({ activeCount: 1, queuedCount: 1, newCount: 1 });
  });

  test("추천 실행은 현재 tier 출처와 모델 평가 ID를 남긴다", async () => {
    const { repository, service: target } = await service();
    const preparation = await collect(
      target,
      "run-provenance-tier",
    ...
    expect(recommendation.ranking[0].companyTier).toBe(1);
    const assessmentId = [...repository.snapshot().companyTierAssessments.keys()][0];
    expect(
      repository
        .snapshot()
        .recommendationTierSources.get(queue.analysisRunId)!
        .get(recommendation.ranking[0].candidateId),
    ).toEqual({ source: "model", assessmentId });
  });
});
