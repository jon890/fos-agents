...
전환을 구현하면서 뒤늦게 드러난 것이다. 이 파일을 만들 때의 목록에 없었다.

**없는 경로에 token 없이 보낸 요청**이다. 기대값은 `401 UNAUTHORIZED` 다.

`err-07-unknown-path` 는 올바른 token 으로 보낸 것이라 `404 NOT_FOUND` 이고,
`err-05-missing-token` 은 실재하는 경로에 보낸 것이다. 둘의 조합이 빠져 있었다.

기대값 `401` 의 근거는 이 파일이 아니라 전환 전 `app.ts` 의 코드 순서다.
`authorize` 가 경로 분기보다 먼저 돌아 token 없이는 모든 경로가 `401` 이었다.
이 차이를 두면 인증 없는 호출자가 응답 코드로 어느 경로가 실재하는지 알아낼 수 있다.
