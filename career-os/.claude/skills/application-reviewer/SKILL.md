...
description: application-package-writer가 생성한 공고별 지원 패키지를 검토해 pass/revise/blocked 판정을 내리는 비공개 career-os skill. "지원 패키지 검토해줘", "리뷰해줘", "review.md 만들어줘", "지원서 점검", `/application-reviewer [application-dir]`처럼 공고별 지원 문서 검토가 필요할 때 사용. 근거 없는 주장, 과장, drift, 쿨다운, 공개 금지 정보, 사용자 승인 필요 항목을 6개 축으로 심사. 실제 제출, 로그인, 채용 사이트 접속, fos-study write는 하지 않는다.
---
