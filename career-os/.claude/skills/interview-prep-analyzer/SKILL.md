...
4. `career-os/sources/fos-study/<path>` — baseline-core-files에 나열된 10개 파일 (각 파일 읽기)
...
6. `career-os/sources/fos-study/<path>` — 선택된 topic의 3-5개 파일 (각 파일 읽기)

### stage 모드 추가

3. `career-os/config/mvp-target.json` — `primary.data_root`, `primary.interview.first_round|final_round|offer_chat`
4. `career-os/<primary.data_root>/interview/prep.md` — 사람이 보는 면접 준비 단일 정본 (있으면 읽는다)
5. `career-os/data/source/<source_dir>/manifest.json`와 사이트 `.txt` 파일 — 수집된 회사/서비스 맥락 (있으면 읽는다)
6. `career-os/sources/fos-study/`의 관련 학습 문서 — 역할/약점과 직접 연결되는 범위만 선별해서 읽는다.
...
1. `study-progress.json`을 읽고 `weak_spots`에서 `last_studied`가 가장 오래되거나 null인 topic-key 선택
...
보고서 쓰기 완료 후 `career-os/config/study-progress.json` 갱신:
...
| topic 자동 선택 실패 (study-progress 없음 + 실제 fos-study 파일 추론 실패) | freeform 모드 — 현재 에이전트가 fos-study에서 적절한 파일 추론 |
...
- **단일 면접 준비 skill (ADR-027, ADR-048)**: baseline + daily + stage는 입력 셋·섹션 수가 다르지만 mvp-target + candidate-profile + 필요한 학습/회사 context 읽기 → 현재 에이전트 분석 → report 쓰기 흐름이 공통이다. 분리 시 SKILL.md drift 위험 — 통합이 agent skill 패턴에 맞다.
