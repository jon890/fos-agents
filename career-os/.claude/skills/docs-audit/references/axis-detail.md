...
Step A 정적 검사 4개 축과 Step B sub-agent 스키마의 구현 상세.
SKILL.md에는 정의·판단 기준만 inline 유지 — 코드·패턴·필터 세부는 이 파일 참조.
...
매치 줄에서 그 경로가 이미 `](path)` 형태로 감싸여 있으면 제외.

### false positive 마스킹 — 백틱 링크 표시 텍스트

`[\`path\`](path)` 형태처럼 백틱 경로가 마크다운 링크의 표시 텍스트인 경우 false positive:
...
- 섹션 참조: `[제목 N장](./file.md#섹션-앵커)` (앵커는 heading의 kebab-case)
...
3. **누적성 섹션의 평탄화 부재** — `## 실제 적용 사례` / `## 관련 사례` / `## 적용 케이스` 헤더 아래 bullet 1개만 있고 그 bullet이 1, 2번 패턴과 겹침
...
1. `,` 콤마 또는 `·` **4회 이상** — 가장 강한 시그널. 자연어에서 4개 이상 항목 enumeration은 거의 항상 줄글이 약함
...
## Step B — sub-agent 표준 YAML schema

모든 sub-agent (orphan-doc-auditor, cross-link-auditor, readme-integrity-auditor) 공통 반환 형식:
...
각 agent 정의 파일에 system prompt가 박혀 있으므로 호출 시 프롬프트는 **간단한 task 트리거**만 필요:

> "fos-study 저장소를 검사하고 표준 YAML schema로 결과를 반환해라."
