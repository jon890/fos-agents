...
**공고를 채점할 수 있는 항목 목록으로 만든다.**
이 목록이 3단계 판정의 단위다. 항목 하나에 판정 하나와 점수 하나가 붙는다.
쪼개는 기준과 그 이유는 [적합도 판정](references/fit-judgment.md)의 「공고 분석」이 소유한다.

시작은 원문 확보다. 공식 페이지에서 공고 원문을 가져와 대상 디렉터리의 `evidence/posting.md` 에 저장한다.
...
항목을 나눌 때 아래 셋은 따로 표시한다. 놓치면 뒤 단계에서 되돌아와야 한다.

- 주요 업무, 기대 경험, 우대 경험 중 어느 구분인지. 구분마다 점수 가중치가 다르다
- 이력서 작성 안내와 전형 단계. 제출물 형식과 준비 범위가 여기서 정해진다
- 공식 문화 기준 중 이 포지션의 판단과 행동을 실제로 가르는 항목
