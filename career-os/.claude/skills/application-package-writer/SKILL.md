...
description: 공고가 찾는 사람과 후보자의 경험, 이력과 방향성이 부합하는지 판정한다. "이 공고 지원 준비", "지원서 준비", "지원 패키지", "지원동기 정리", "이 회사에 지원하고 싶어", "이 회사에서 뭘 배울 수 있어", "어느 공고부터 지원할까"처럼 개별 공고의 지원 전략이 필요할 때 사용한다. 이력서·경력기술서 작성과 검증은 resume-preparer로 연결한다. 실제 제출, 로그인과 외부 전송은 하지 않는다.
---
...
| --- | --- | --- | --- |
| 1 | 공고 해체 | 공고 항목을 컴포넌트 단위로 쪼갠 목록과 각 항목의 구분 | [적합도 판정](references/fit-judgment.md) |
| 2 | 근거 수집 | 각 항목에 연결할 후보자 근거와 그 근거를 확인한 위치 | [적합도 판정](references/fit-judgment.md) |
| 3 | 적합도 판정 | 항목별 판정과 구분별 소계, 총점 | [적합도 판정](references/fit-judgment.md) |
| 4 | 후보자 인터뷰 | 후보자만 확정할 수 있는 동기, 역할, 제약과 결과 범위 | [후보자 인터뷰](references/candidate-interview-questions.md) |
| 5 | 경험과 성장 판정 | 서비스가 커질 여지, 경쟁 위치, 경험할 트래픽과 얻기 어려운 것 | [경험과 성장 판정](references/growth-judgment.md) |
| 6 | 지원 전략 작성 | 승부처, 지원동기, 기여 시나리오와 공백 보완 계획 | [지원 준비 품질 기준](references/application-quality-rubric.md) |
| 7 | 사용자 검토 | 동기, 역할 구분, 핵심 사례, 공백과 기여 시나리오의 사용자 확인 | [지원 문서 전수 검토](references/full-document-review.md) |
| 8 | 제출 문서 연결 | `resume-preparer` 호출과 준비 상태 | [이력서 제출 준비](../resume-preparer/SKILL.md) |

다음 두 조건을 모두 충족해야 다음 단계로 간다.

- 그 단계에서 확인하기로 한 것이 남지 않았다.
- 사용자만 답할 결정이 남지 않았다.

단계 1부터 3까지가 이 스킬의 목표를 직접 수행한다.
단계 3의 판정이 나오기 전에는 지원동기와 승부처를 쓰지 않는다.

단계 5는 단계 4 뒤, 단계 6 앞에 둔다.
얻기 어려운 것을 판정하려면 후보자가 밝힌 약한 축이 있어야 하고, 그 축은 단계 4의 「약한 영역과 보완 방법」이 소유한다.
결과가 「보완할 공백」 바로 앞 절로 들어가므로 지원 전략을 쓰기 직전이 자리다.
...
**통과 조건:** 공고 원문의 모든 항목이 목록에 있고, 한 항목에 컴포넌트가 둘 이상 남아 있지 않다.
...
부합하지 않으면 지원하지 않는 이유를 남기고 제출 문서 작성으로 연결하지 않는다.
