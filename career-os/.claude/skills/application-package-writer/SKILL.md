...
적합도 판정이 공고 요구와 후보자 근거가 맞닿은 정도를 재는 것과 달리, 이 판정은 지원 여부와 지원 순서를 정하는 근거를 만든다.
...
「공개 자료로 확인한 팀과 인접 사례」와 자료 출처가 겹치므로 그 reference의 「이웃 절과의 경계」로 무엇을 어느 절에 둘지 정한다.
...
**여기 적힌 것으로 끝낸다.** 실측으로 셋이 반복해서 늘어났다.
claim ledger 를 풀어 쓴 evidence audit, 공통 디자인을 복제한 공고별 design 문서, 이전 제출 PDF 다.
