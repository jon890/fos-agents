...
부합하지 않으면 막은 항목과 메우는 방법을 남긴다. 3단계가 그 형식을 정한다.
...
| --- | --- | --- | --- |
| 1 | 공고 해체 | 공고 원문을 채점할 수 있는 항목 하나하나로 쪼개고 어느 구분에서 왔는지 붙인다 | [적합도 판정](references/fit-judgment.md) |
...
**공고 전체를 한 덩어리로 판정하면 어느 요구를 채우고 어느 것을 못 채우는지 알 수 없다.**
그래서 먼저 채점할 수 있는 크기로 쪼갠다. 한 항목에 요구가 둘 이상 섞여 있으면 그 항목은 판정할 수 없다.
