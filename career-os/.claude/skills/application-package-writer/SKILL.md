...
| 쪼갠 항목 목록 | `evidence/application-package.md` 의 「공고 항목별 적합도」 표 첫 열 | 한 행에 요구 하나. 2단계가 근거 열을, 3단계가 판정 열을 채운다 |
...
**규모 수치가 공고에 없으면 없다고 적는다.** 5단계가 그 사실을 근거로 쓴다.
...
결과는 `evidence/application-package.md`의 「이 자리에서 얻을 경험과 성장」에 남긴다.
