...
| --- | -------- | --------------------------------------------- | ------------------------------------------------------- |
| 1   | 공고 분석    | 공고 원문을 요구 하나가 한 항목이 되도록 쪼개고 어느 구분에서 왔는지 붙인다   | [적합도 판정](references/fit-judgment.md)                    |
...
그래서 요구 하나가 한 항목이 되도록 쪼갠다.

공고는 한 줄에 요구를 여러 개 담는 경우가 많다.
「Kafka 기반 비동기 처리와 MySQL 쿼리 튜닝 경험이 있으신 분」은 두 항목이다.
한 줄로 두면 Kafka 는 되고 튜닝은 안 될 때 판정 하나로 답할 수 없다.

공고 원문은 대상 디렉터리의 `evidence/posting.md` 가 담는다.
처음 지원하는 공고면 이 파일이 없다. 공고 URL 에서 원문을 가져와 공식 URL 과 확인 시각을 함께 적어 만든다.
이미 있으면 그것을 읽고, 공식 페이지에서 공고가 지금도 열려 있는지 다시 확인한다.
