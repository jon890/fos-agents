...
스크립트는 Backend에 소스, 수집 자료, 추천 이력과 제외 판정을 저장한다.
...
게시된 리포트가 추천 이력에 없다면 최신 상태라고 단정하지 않는다.
확인 가능한 Pages 리포트와 추천 이력을 대조해 누락과 반복 추천을 알린다.
...
YouTube도 글과 함께 검토한다.
최근 추천에서 영상이 계속 빠졌다면 채널별 수집 결과와 제외 이유를 확인한다.
...
선택하지 않은 모든 후보는 `rejections`에 후보 ID와 한 줄 이유를 함께 낸다.
이 제외 판정이 있어야 다음 실행에서 같은 후보를 다시 평가하지 않는다.
...
관심사가 바뀌면 `configure_study_recommendation.ts`로 후보자 기준 버전을 올린다.
...
검증된 결과만 Backend 추천 이력에 반영한다.
추천 저장이나 외부 게시 기록이 실패하면 복구에 필요한 파일을 보존하고 실패 지점을 알린다.
