...
- `preferenceExcluded`(config `verified-company-research-targets.json`) 회사는 사용자가 크게 가고 싶은 회사로 보지 않는다(ADR-094). 해당 개별 공고는 강력 추천/도전 추천/이번 주 즉시 지원 액션에서 제외하고, 필요하면 보류·제외 사유로만 짧게 언급한다.
...
- 선호 제외 대상은 config `preferenceExcluded`를 단일 출처로 본다(ADR-094). JD fit이 높아도 회사 선호가 낮으므로 추천 티어에 올리지 않는다.
