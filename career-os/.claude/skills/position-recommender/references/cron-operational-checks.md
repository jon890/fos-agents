...
3. 전체 후보 preview HTML을 생성한다.
   - `node scripts/position-recommender/render_candidate_preview.ts --input data/runtime/position-recommendation.json --postings data/runtime/live-position-postings.md --limit all --output data/runtime/downloads/position-recommendation-preview-all-$(TZ=Asia/Seoul date +%F).html`
...
5. HTML 전달 산출물을 확인한다.
   - 전체 리포트 HTML: `data/runtime/downloads/position-recommendation-full-YYYY-MM-DD.html`
   - 전체 후보 preview HTML: `data/runtime/downloads/position-recommendation-preview-all-YYYY-MM-DD.html`
...
- `--limit 50`처럼 임의로 자르지 않는다. 사용자가 전체 후보를 원하면 `--limit all`을 쓴다.
