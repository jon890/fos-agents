...
- `career-os:daily-server-position-recommendation` 예약 실행의 prompt, workdir, delivery, HTML 게시 방식을 바꾼 뒤.
...
- 텍스트 표만 만들고 게시 준비용 HTML을 빼먹지 않는다.
- `--limit 50`처럼 임의로 자르지 않는다. 게시 준비용 HTML은 `--limit all`을 쓴다.
