# 포지션 리포트 HTML 게시와 전체 공고

## 사용자 선호

- 공고·포지션 추천 리포트는 텍스트 표만 보내지 않는다.
- 항상 게시 준비용 HTML을 함께 만든다.
- 게시 준비용 HTML은 **추천 공고와 전체 조건 통과 공고를 한 파일에 함께** 담는다. 파일을 두 개로 나누지 않는다.
- HTML 안의 공고명은 개별 공고 URL로 이동하는 링크여야 한다.
- Discord 미리보기에도 상위 후보, 핵심 사유, 공고 링크를 포함한다.
- 게시 준비용 HTML은 임의로 20개·50개 등으로 자르지 말고 전체 active/open 후보를 보여준다.
- 외부 게시나 공유 URL 생성을 명시적으로 요청하면 `report-publisher`로 공개 범위를 점검하고 검증된 URL을 전달한다.
...
- **추천 공고**: 강력·도전·보류 티어 전체. 티어 뱃지를 함께 보여준다.
  수집 snapshot 밖에서 직접 확보한 공고도 여기에 들어가므로, 이 섹션이 없으면 신규 발굴 공고가 게시본에서 누락된다.
...
- 텍스트 답변은 요약 수준으로 두고, 외부 공유가 승인되면 검증된 HTML 게시 URL을 전달한다.
...
- `reports/downloads/` 밖의 HTML을 `report-publisher` 입력으로 사용하지 않는다.
