...
- Discord 요약에는 강력 추천 최대 3개, 도전 추천 최대 2개를 짧게 쓰되 첨부 HTML은 항상 `--postings data/runtime/live-position-postings.md --limit all`로 전체 active/open 후보를 보여준다.
- 전체 공고 HTML에서도 AI 모델 연구 중심 포지션과 CTO/기술총괄 포지션은 제외한다.
- 전체 공고 HTML은 다음 4줄 또는 표 컬럼을 유지한다:
  - 공고명: 회사명 — 포지션명
