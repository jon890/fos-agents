...
- 비정형 면담 요청, 학습 갭, 면접 준비 진단은 `interview-prep-analyzer`로 라우팅한다.
...
8. `references/verified-company-research-targets.json` — 검증된 회사 탐색 대상 목록. 추천 티어 입력이 아니라, active 개별 공고가 부족할 때 다음 수집 범위 결정에만 사용한다.
...
- **강력 추천 포지션** 티어 — role-fit 높고 gap 준비 가능한 포지션
  - 최대 3개
  - 각 항목: role title, 개별 active 포스팅 링크, 공고 기간, 지원 근거 1~2문장, gap 준비사항, first action
- **도전 추천 포지션** 티어 — stretch goal, 준비 기간 필요
  - 최대 2개
  - 각 항목: role title, 개별 active 포스팅 링크, 공고 기간, 지원 근거 1~2문장, gap 준비사항, first action
- **추가 수집 대상** — 개별 active 공고 URL을 아직 확보하지 못한 회사·직무 lead
  - 최대 3개
  - 이 섹션은 다음 수집 범위 결정용이며 사용자에게 직접 지원하라는 뜻이 아니다.
- **보류·주의 포지션** 티어 — 현시점 비추천 + 사유 명시
  - 최대 3개
  - 각 항목: role title과 비추천 사유
- **최근 반복 점검** — 최근 7일 반복 후보와 신규 후보 확보 여부
- **이번 주 액션 플랜** — 지원/검색, 이력서 보강, 학습 액션을 각각 1개 이내로 쓴다.
- 판단 근거를 충실히 담는 것을 우선한다. 줄 수 상한은 강제하지 않으며, 다만 장문 회사 설명·JD 원문 재서술은 쓰지 않는다.

추천 티어 항목은 runner가 아래 라벨을 파싱하므로 정확히 유지한다.
...
```
쓰기 → career-os/data/reports/daily/YYYY-MM-DD/position-recommendation/report.md
쓰기 → career-os/data/runtime/position-recommendation.md  (런타임 미러)
...
Runner post-process → career-os/data/reports/daily/YYYY-MM-DD/position-recommendation/items.json
Runner post-process → career-os/data/runtime/position-recommendation-items.json
Runner post-process → fos-career DB recommendation items ingest
```
...
`items.json`과 DB ingest는 runner 책임이다.
skill은 Markdown report와 runtime mirror를 정확한 형식으로 만드는 데 집중한다.
...
리포트 작성 후 자기 출력 검증 17항목 (옛 `extract_position_report.ts` 45줄 흡수 + stale-output 방지):

1. 첫 줄 `# ` 시작 (단일 `#`, `## ` 시작 금지)
2. 총 줄 수 ≥ 30
3. **강력 추천 포지션**, **도전 추천 포지션**, **보류·주의 포지션** 3 티어 헤더 모두 존재
...
17. 강력 추천/도전 추천의 모든 항목에 `후보자 경험 근거`, `JD에서 노려야 할 키워드`, `회사/규모 업사이드`, `복지/학습 환경 판단`, `기술블로그/엔지니어링 시그널`, `사업/조직/seniority 리스크` 6개 라벨이 모두 존재함

실패 항목 있으면 해당 섹션 보완 후 재작성. **최대 3회 시도**.
4회째도 실패 시 `stderr: position-recommender 검증 실패: <실패 항목>`를 출력하고 exit 1로 종료한다.
