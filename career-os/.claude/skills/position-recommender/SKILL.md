...
목록은 공개 저장소에 두지 않고 `state/private-config/` 에 두며 이 명령이 원격 release 에서 내려받는다.
...
| 종료 코드 | 뜻         | 처리                                                    |
| ----- | --------- | ----------------------------------------------------- |
| 0     | 후보풀 생성 성공 | `WARN collection health` 줄이 있으면 누락 소스를 사용자에게 알리고 계속한다 |
...
| 값                                    | 소스                                          | 없을 때                                                                     |
| ------------------------------------ | ------------------------------------------- | ------------------------------------------------------------------------ |
| 현재 직장의 값. 문제의 난도, 오너십, 도메인 확장 여지, 보상 | private brain `career-position-preferences` | 그 축을 `확인 필요`로 두고 리포트에 그 사실을 적는다                                          |
| 역할 선호와 관심 도메인                        | private brain `career-position-preferences` | 자격과 스택 일치로만 정렬하고 리포트에 그 한계를 적는다                                          |
| 지원 이력과 재지원 간격                        | private brain `career-status`               | 추정하지 않는다                                                                 |
| 추천에 올릴 회사의 연봉 구간                     | 크레딧잡과 원티드. 판정 기준 문서가 소유한다                   | 보상 축을 `확인 필요`로 두고 무엇을 못 찾았는지 적는다                                         |
| 후보자 경력 근거                            | `sources/fos-study/task/`. 읽기 전용이다          | 이 경로는 git 추적 대상이 아니라 없을 수 있다. 없으면 private brain 근거로만 판정하고 그 사실을 보고서에 적는다 |


brain 조회는 `brain-search` 를 쓴다.

**brain 에 있는 값을 다시 묻지 않는다.** 없으면 판정을 멈추지 않고 그 한계를 리포트에 적는다.
...
이 명령이 `autoExclusionSuggestions` 를 읽어 제외 공고 목록에 더한다.
같은 공고가 이미 목록에 있으면 건너뛴다.

**종료 코드를 확인한 뒤 다음으로 간다.** 목록 갱신이 실패한 상태로 release 를 올리면 원격이 잘못된 상태가 된다.

```bash
CAREER_WORKSPACE_ROOT="$PWD" bun scripts/career-workspace/cli.ts skill finish position-recommender --json
```

release 반영이 실패하면 로컬 설정을 지우지 않는다. 충돌 내용과 로컬 설정을 보존했다는 사실을 사용자에게 알린다.

### 6. 보고서

그날 데이터에 맞는 정보 구조와 시각 구성을 골라 `<RUN_DIR>/index.html` 하나를 만든다. 지원 순위와 근거, 위험, 확인할 사항, 다음 행동, 공개 공고 링크가 드러나야 한다.
...
**이 검사는 공개 계약 검사다.** 잡는 것은 `현재 연봉` 과 `서류 탈락` 문자열, 로컬 절대 경로, 로컬 호스트, HTTPS 가 아닌 링크다.

**이 검사가 막는 것은 공고가 아니라 근거 문장이다.**
순위를 정할 때 지원 이력과 현재 보상을 근거로 쓰는데, 그 근거를 리포트에 그대로 옮기면 개인 정보가 공개된다.
공고 자체는 공개된 것이라 보류 목록에 그대로 둬도 된다.

**검사기가 통과했다고 공개해도 된다는 뜻이 아니다.**
외부 소스에서 가져온 연봉 수치와 `서류 탈락` 이외의 표현으로 적은 지원 결과는 검출되지 않는다.
지원 이력을 근거로 순위를 내렸다면 「같은 법인에 최근 지원 이력이 있어 재지원 시점을 확인해야 한다」처럼 결과를 적지 않고 쓴다.
모델이 만든 HTML 이 검사를 통과하지 못할 때만 고정 템플릿으로 다시 렌더하고 검사한다.
...
이번 실행에서 만든 파일을 확인해 삭제하고 빈 `<RUN_DIR>`을 `rmdir`로 지운다.
공개 Pages 배포와 비공개 release는 그대로 둔다.
