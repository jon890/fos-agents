...
daily/cron은 `scripts/position-recommender/run_daily_with_claude.sh`가 에이전트 호출 전에 `data/runtime/live-position-postings.md`를 먼저 갱신한다.
agent skill은 이 파일을 우선 읽는다.
...
agent skill 내부에서는 Discord 알림을 직접 보내지 않는다.
...
이는 OpenClaw/Codex가 실행 조율과 외부 전송을 맡고,
agent skill은 리포트 생성·자기 검증에 집중하게 하기 위함이다.
...
- **self-check 내재화**: `extract_position_report.ts`가 하던 첫 줄 `#` + 줄 수 검증을 현재 에이전트 자체 검증으로 흡수. 외부 프로세스 불필요.
- **수집 선택적 호출**: 기존 `POSITION_POSTINGS_FILE` env 주입 패턴 → 자연어 인자 흡수. 매번 수집하지 않아 비용·시간 효율.
- **env 변수 제거**: `POSITION_CONTEXT` + `POSITION_POSTINGS_FILE` → 자연어 인자.
