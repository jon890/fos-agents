...
description: >
  매일 인성 면접 질문(STAR·가치관)에 1문장으로 답하고 채점·약점 환류하는 career-os 대화형 드릴 skill.
  공용 드릴 엔진(scripts/interview-drill/drill-engine.ts)을 재사용한다.
  ...
  기술 면접 드릴은 tech-interview-drill, 단계별 면접 준비는 interview-stage-prep 담당.
---
...
사용자 답변을 받으면 `references/scoring-rubric.md`의 STAR·가치관 두 축으로 채점한다:

| 결과 | 기준 | 피드백 형식 |
|------|------|-------------|
| ✅ 통과 | STAR 4요소 + 가치관 연결 명확 | "STAR 4요소 확인. 가치관 연결 포인트: … 보완: …" |
| ⚠ 얕음 | STAR 또는 가치관 중 하나가 얕음 | "방향은 맞으나 얕음. 보완이 필요한 축: … 추가로 서술해야 할 것: …" |
| ❌ 틀림 | STAR 누락 또는 가치관 미드러남 | "핵심 놓침. STAR 누락 요소: … 가치관 문제: … 핵심 답변 방향: …" |
...
`drill-engine.ts`의 `REVIEW_INTERVALS_DAYS = [1, 3, 7, 14, 30, 60]` 기준 (tech 드릴과 동일):
...
- 위임 입력: 사용자가 언급한 경험 키워드 + 현재 드릴 역량 범주.
