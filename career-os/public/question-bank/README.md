...
질문 보강은 기존 bank만 재배열하지 않고 공개 소스를 함께 확인한다.
기술 질문은 공식 문서, 공식 기술 블로그, 공개 기술 발표, 공개 개발자 회고를 우선 근거로 삼는다.
인성 질문은 공식 채용·컬처 자료, 채용 플랫폼 후기, 공개 개인 회고를 함께 확인한다.
IT 기업 자료를 수집할 때는 네이버, 카카오, LINE, 쿠팡, 토스, 우아한형제들 등 여러 회사의 자료를 섞어 특정 회사에 과적합하지 않은 일반 질문으로 정규화한다.
