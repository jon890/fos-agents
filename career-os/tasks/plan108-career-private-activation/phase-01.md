...
**선행 조건**: `plan107-career-workspace-portability`가 merge되고 client 계약 테스트가 통과해야 한다. 홈서버의 `plan2-career-file-storage`가 배포되어 `career-storage status` 계약을 제공해야 한다.
...
기본 후보는 `career-os/applications`, `career-os/private`, `career-os/state`, 과거 `career-os/data/applications`다.
사용자가 명시한 legacy root만 추가하며 cache, 공개 임시 리포트와 다른 workspace는 탐색 범위에 넣지 않는다.
...
- plan107이나 홈서버 저장 계약이 준비되지 않았으면 `PHASE_BLOCKED: 선행 release 계약 미충족`으로 끝낸다.
