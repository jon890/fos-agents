...
- `scripts/position-recommender/candidate-store/` 신규
...
- `cache/`에 재생성 가능한 공고 원문을 두고,
  `state/`에 버전이 있는 분석 결과를 두면 현재 release 구조를 유지할 수 있다.
- MySQL을 다시 사용하면 여러 서비스의 조회와 동시 갱신은 쉬워지지만,
  외부 DB를 필수 구성 요소에서 제외한 ADR-102를 다시 결정해야 한다.
...
MySQL 안은 `fos-assistant`의 DB migration과 API 계약이 필요하다.
해당 저장소는 다른 세션의 계획 범위이므로 이번 Dispatch에서는 파일을 열거나 고치지 않았다.
...
Hermes에서 새 세션이나 도구가 적은 모델 호출을 시작하는 부분은 `fos-assistant` 계획과 합의해야 한다.
...
매일 반복 실행한다는 조건을 반영해 안 B의 파일 기반 저장과 우선순위 큐를 먼저 구현한다.
같은 phase에서 안 A의 전용 준비와 최종화 명령을 붙여 모델이 임시 코드를 만들거나
추천 검증 뒤에 여러 도구를 왕복하지 않게 한다.
실제 분석 재사용 비율과 동시 접근 요구를 측정한 뒤 여러 서비스가 같은 데이터를 조회해야 할 때만 MySQL 전환을 결정한다.
