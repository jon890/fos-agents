...
지금 `SKILL.md`는 161줄에 명령 블록이 5개이고 「목표」 절이 없다.
무엇을 전달하는 skill인지 한 줄도 적혀 있지 않다.

앞선 계획이 파일 동기화를 없애 명령 블록이 17개에서 5개로 줄었다.
Phase 01이 진입점을 넷으로 모았다.
그래서 지금 문서가 설명하는 절차 대부분이 이미 사라졌거나 한 명령 뒤에 있다.
...
- 「목표」 절이 있다
- 명령 블록이 두 개 이하다. 하위 명령 넷은 한 블록에 담는다
- 전체가 110줄 이하다
...
**110줄 이하이고 명령 블록이 2개 이하여야 한다.**
시작값은 161줄에 5개 블록이다. 이 값을 넘기지 못하면 절차를 덜 옮긴 것이다.
