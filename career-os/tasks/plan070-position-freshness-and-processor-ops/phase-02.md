...
검증이 끝나면 fos-career repo에서 의도한 wrapper 변경만 stage한다.
commit 메시지는 `fix(processors): 포지션 액션 host-side wrapper 추가`를 사용한다.
career-os repo의 task 상태 파일은 stage하지 않는다.
