...
**이 phase가 ledger rename보다 먼저다** — frontdoor 파일(`frontdoor_queue_schema.ts`·`promote_frontdoor_candidate.ts`)이 `./ledger_io`·`./ledger_schema`를 import하므로, frontdoor를 먼저 제거해야 Phase 07 ledger rename에서 dangling import가 생기지 않는다. 이 시점 ledger 파일은 옛 이름 그대로 존재한다.
