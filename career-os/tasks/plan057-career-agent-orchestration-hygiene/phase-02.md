...
- HUD 구현 코드 수정.
- canonical state root를 제외한 OpenClaw state 삭제는 후속 정리에서 수행.
- cron/systemd 설정 변경.
- 후속 phase 구현.
...
- 옛 HUD state root는 존재 여부만 확인하고, 현재 운영 경로는 `.openclaw/workspace-career/state/task-hud`로 본다.
...
3. 옛 HUD state root 처리 선택지를 적는다.
