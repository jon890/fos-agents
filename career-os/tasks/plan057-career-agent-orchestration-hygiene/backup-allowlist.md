...
이 문서는 career OpenClaw workspace 파일을 백업할 때의 allowlist 후보와 제외 대상을 정리한다.
...
백업은 allowlist 방식으로만 검토한다.
즉, "전체 workspace 복사 후 제외"가 아니라 "보존할 파일만 명시해 복사"한다.

백업 후보는 다음 기준을 통과해야 한다.

- 다음 세션에서도 재사용해야 하는 운영 지침인가?
- career-os wrapper ownership, HUD/status판 운영, registry smoke 절차처럼 반복 적용되는 규칙인가?
...
- `skills/*/SKILL.md`
  ...
  - 단, private prompt 본문이나 token이 섞이지 않았는지 별도 review한다.
- `skills/*/references/`
...
- `career-os/tasks/plan057-career-agent-orchestration-hygiene/ownership-inventory.md`
  - wrapper ownership 경계와 migration 후보.
- `career-os/tasks/plan057-career-agent-orchestration-hygiene/hud-protocol.md`
  - `session_status` 우선 HUD/status판 갱신 절차와 stale snapshot 실패 모드.
- `career-os/tasks/plan057-career-agent-orchestration-hygiene/session-registry-flow.md`
  - agent switch 뒤 session registry smoke, main session, background implementation 책임 분리.
...
HUD/status판에는 safe summary만 쓰고, 백업 대상에도 raw session payload를 넣지 않는다.
...
- commit history로 ownership, HUD protocol, session registry flow 변경을 추적할 수 있다.
- allowlist가 잘 고정되면 agent workspace 재설정 시간이 줄어든다.
...
- private repo를 만들려면 먼저 allowlist manifest를 고정한다.
...
- allowlist 없이 통째로 압축하면 session logs, cache, tokens, private secrets가 섞일 위험이 커진다.

권장 기본값:

- local archive도 allowlist manifest를 사용한다.
