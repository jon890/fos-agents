...
| `data/private/` | private-only 보관과 archive home. 공개·제출·runtime 경계가 애매한 민감 자료의 임시 보존 위치 | ✗ |
...
- `data/private/` — public/source/report/runtime 중 어느 곳에도 바로 둘 수 없는 private-only 보관과 archive home.
  오래된 민감 자료는 삭제보다 이 위치 아래의 날짜·plan·주제별 archive로 옮기는 결정을 먼저 남긴다.
...
- 오래된 generated report는 최근 7일 반복 추천, active interview/application 판단, task/ADR 근거와 연결돼 있으면 보존한다.
  연결이 없고 새 report나 docs 결정으로 대체됐으면 `data/private/archive/` 후보로 분류한다.
...
- coffeechat 자동화 tombstone은 삭제 대상이 아니라 archive/tombstone 결정 대상이다.
  active workflow 제거 여부와 과거 이력 보존 여부를 분리해 판단한다.
