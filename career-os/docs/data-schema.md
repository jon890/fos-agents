...
읽을거리 실행은 후보풀, 현재 선별 결과, 이력 파일을 `state/`에 만들 수 있다.
이 파일들은 소스에서 다시 만들 수 있는 로컬 실행 자료이며 공개 자료가 아니다.
...
수집 후보는 외부 원문 URL, 출처, 제목, 게시 시각, 요약 가능한 본문을 포함한다.
