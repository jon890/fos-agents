...
│   │   ├── recommendation_schema.ts    산출물 정본 zod 스키마 (ADR-093, RecommendationRun)
│   │   ├── render_recommendation.ts    정본 recommendation.json에서 Markdown·HTML 파생 (ADR-093)
...
Claude와 Codex는 같은 SKILL.md를 공유하고, Codex 발견 경로는 `.codex/skills` 심볼릭 링크를 사용한다.
...
- `position-recommender` native skill은 산출물 정본 `recommendation.json`(ADR-093, schemaVersion 2)을 만든다.
- `scripts/position-recommender/render_recommendation.ts`는 정본 JSON에서 Markdown·HTML을 파생한다(입력 시 zod 검증 내장). 자체 markdown 파서 `render_report_html.ts`는 ADR-093으로 폐기됐다.
