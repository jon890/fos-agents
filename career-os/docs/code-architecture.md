...
│   - 10개: study/interview/application/position 계열          │
└────────────────┬────────────────────────────────────────────┘
                 │
┌────────────────┴────────────────────────────────────────────┐
│ 스킬별 스크립트 (필요 시)                                    │
...
│   │   └── SKILL.md  (plan017에서 agent skill 명세 작성. plan041에서 first-round/final-round/offer-chat 준비 이관)
│   ├── study-topic-recommender/
...
# Claude CLI 호환 실행 경로가 필요한 경우에도 같은 skill 호출 의도를 전달한다.
...
├── skill_executor.ts              # --execute-skills 명시 시 agent-only private skill 실행
├── progress_notifier.ts           # --notify-discord 명시 시 private-safe progress 알림
├── skill_contracts.ts             # agent skill call contracts + CLI command builder (phase-04)
...
| 책임 | 담당 |
|---|---|
| 분석, 작성, 리뷰, 추천 근거 생성 | agent skill (LLM) |
...
| 명시 옵션에서 agent skill 실행 | TypeScript `skill_executor.ts` (`--execute-skills`) |
...
- career-os agent skills: 기존 `interview-prep-analyzer`, `interview-asset-writer`, `study-pack-writer`를 재사용한다.
