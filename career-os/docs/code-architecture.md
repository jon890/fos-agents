...
│   └── interview-coffeechat-prep/{collect_company_sites.ts}
      (deprecated tombstone — coffeechat 자동화 폐기, ADR-048)
│
...
│   ├── interview-coffeechat-prep/{SKILL.md, references/}
   │   (deprecated tombstone — active workflow에서 제거, ADR-048)
...
- `app/api/chat/route.ts`는 특정 SDK를 직접 호출하지 않고 `lib/llm/provider.ts`의 `createLlmProvider()`만 사용한다.
- `LLM_PROVIDER=openai`는 OpenAI Responses API provider를 사용한다.
- 현재 운영 provider는 OpenAI/Codex 계열만 허용한다.
- 새 SDK는 `LlmProvider.streamText()` 계약만 맞추면 추가한다.
...
- PDF/DOCX export는 Markdown contract가 안정된 뒤 후속 plan에서 다룬다.
