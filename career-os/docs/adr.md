...
- 기본 provider는 `LLM_PROVIDER=anthropic`이고, `LLM_PROVIDER=openai`로 OpenAI Responses API provider를 선택할 수 있다.
...
- Anthropic, OpenAI, 후속 SDK 구현을 같은 `streamText()` 계약으로 교체할 수 있다.
