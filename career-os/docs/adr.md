...
- `study-pack-writer` 요청은 기존 방식처럼 `sources/fos-study/`에 `[초안]` 제목의 공부팩을 즉시 생성하고 commit/push까지 수행한다.
  push 실패는 요청 실패로 남긴다.
...
특히 Coupang 공식 careers는 현재 fetch 기반 수집에서 Cloudflare 차단을 반환한다.
...
- 새 source는 `--source all` shadow 검증을 먼저 거치고, daily 기본값 전환은 별도 판단으로 둔다.
