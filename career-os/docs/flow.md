...
```
config/study-pack-topics.json (primary)
...
config/ai-topic-sources.json
config/geek-news-sources.json
  ↓
...
```
config/study-topic-candidates.json 정리:
...
candidate reservoir가 목표치 이하면:
  ...
  ↓
검증 통과 후보만 config/study-topic-candidates.json에 append
  ↓
...
```
config/study-pack-topics.json 또는 study-pack-maintainer-topics.json에서 topic 정보 조회
  - maintainer config에 있으면: maintainer 경로 (기존 갱신, ADR 미정 — 라우팅 결정)
  - primary config에 있으면: writer 경로 (신규 생성)
...
```
config/study-pack-maintainer-topics.json에서 topic 조회
  ↓
...
```
config/experience-question-bank-topics.json에서 topic + inputFiles 조회
  ↓
