...
2. `state/morning-study-history.json`에서 이전에 추천한 자료의 `contentKey`를 읽는다.
...
8. 선택 검증은 후보풀에 없는 자료, 카테고리 불일치, 실행 내 중복과 이전 추천의 재선택을 거부한다.
9. 같은 선별 결과에서 주제 중심 Markdown과 HTML을 만들고 공개 범위와 링크를 검증한다.
