...
3. 기존 후보자 인터뷰 답변을 읽고, 동기, 당시 제약, 본인 판단, 기각한 대안과 확인하지 못한 결과 중 비어 있는 독립 질문을 최대 6개까지 묶어 확인한다.
...
8. `application-package.html`의 첫 영역에는 현재 할 일, 제출 후보 PDF와 필요한 경우의 지원서 입력값만 보여준다.
9. 승부처, 지원동기, 공백과 다음 행동을 먼저 보여주고 회사 기준, 전체 근거, 기여 시나리오, 이력서 원문, 개별 PDF, 면접 질문과 후보자 인터뷰 기록은 닫힌 상세 영역에 묶는다.
10. 사용자는 이 화면에서 지원동기, 소유권, 가장 강한 사례, 공백과 입사 후 기여 시나리오를 검토하고 대표 사례를 30초와 2분으로 설명할 준비가 됐는지 확인한다.
...
8. 코드 사용과 기능 개발만 확인되고 운영 노하우가 불명확하면 서로 독립적인 확인 질문을 최대 6개까지 묶는다.
9. 근거가 약한 문장은 삭제하거나 확인 가능한 표현으로 낮춘다.
10. 사용자 확인이나 수정 판정이 남으면 최종 평가를 시작하지 않는다.
11. 공고와 제출 PDF만으로 블라인드 채용 담당자·기술 리더 판정을 먼저 만든다.
