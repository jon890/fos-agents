...
[매일 아침]
  ↓
  claude --permission-mode bypassPermissions -p "/study-topic-recommender"  → data/runtime/morning-topic-recommendation.md
  ↓ (10픽 + 오늘의 3선)
  사용자가 1개 토픽 선택
  ↓
...
Bash: bun career-os/scripts/position-recommender/collect_live_postings.ts
  ...
  → source adapters (Wanted broad/target URL/target keyword, Toss job-groups, Coupang Careers sitemap, KakaoPay, KakaoPay Securities, KakaoMobility, NAVER Careers 등)
...
Post-process: data/reports/daily/YYYY-MM-DD/position-recommendation/report.html
...
AI Agent/RAG/MCP/LLMOps/ML Backend 같은 백엔드 + AI 전환 후보는 Wanted target URL과 AI keyword search로 별도 수집한다.
...
### `study-topic-recommender` (모닝 추천 — native skill, ADR-026 + ADR-033 + ADR-070)

native skill 패턴: `claude --permission-mode bypassPermissions -p "/study-topic-recommender"` → SKILL.md 자동 로드 → Claude가 도구로 직접 처리.

on-demand 실행에서는 후보 제안 JSON, runtime report, inventory markdown을 쓰고 `Bash`로 Bun 스크립트를 실행한다.
기본 `claude -p` 또는 `acceptEdits`만 쓰면 비대화형 실행 중 권한 승인 요청에서 멈출 수 있으므로,
...
```
호출: claude --permission-mode bypassPermissions -p "/study-topic-recommender"
  ↓
...
`claude --permission-mode bypassPermissions -p "/study-topic-recommender live-coding 1개 골라줘"` — study-topic-recommender가 live-coding seed 선택을 내부적으로 처리.
