...
4. 지원 판단과 근거를 `application-package.md`에 통합하고 제출용 이력서 초안을 만든다.
5. 두 원본과 후보자 인터뷰 기록을 `application-package.html` 하나로 묶는다.
6. 사용자는 이 화면에서 지원동기, 소유권, 가장 강한 사례, 공백과 입사 후 기여 시나리오를 검토한다.
7. 자동 상태 전이나 독립 검토가 필요하면 내부 reviewer가 패키지 계약과 과장, 누락, 공개 범위를 검사한다.
8. 준비 상태는 `ready`, `needs_user_input`, `revise`, `do_not_apply` 중 하나로 남긴다.
9. `ready`여도 실제 제출은 사용자 승인 전까지 수행하지 않는다.

## 이력서 근거 감사와 개선

이력서 문장을 실제 업무 근거에 연결하고 HTML 결과를 반복 개선한다.

1. `resume-draft.md`와 디자인 계약으로 두 페이지 HTML과 PDF를 만든다.
...
10. 두 검토자가 모두 통과하고 경쟁상 차단 항목이 없을 때만 제출 준비가 끝난다.
11. 정적 검사와 실제 브라우저 렌더링으로 페이지 수와 잘림을 확인한다.
12. 보이는 문구가 바뀌면 근거 감사를 다시 실행한다.
