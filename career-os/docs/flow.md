...
`data/` 아래 흐름은 ADR-058의 private by default 원칙을 따른다.
정리 작업은 삭제보다 archive, tombstone, retention 검토를 먼저 수행한다.

```text
source 수집 또는 agent 실행
  -> data/source, data/runtime, data/reports, data/applications 중 책임 위치 선택
  -> 지원/면접/후보자 맥락과 연결됐는지 확인
  -> 연결됐으면 private by default
  -> 오래됐거나 active 판단에서 빠졌으면 retention 검토
  -> 삭제 전 data/private/archive 또는 tombstone 후보로 분류
```
...
- `data/runtime/`은 최신 projection, cache, lock, eval 결과를 둔다.
  장기 근거가 되면 runtime에 남기지 말고 `data/reports/`, task evidence, `data/private/archive/` 중 하나로 승격 결정을 남긴다.
...
- `data/private/`는 삭제 전 archive/tombstone home이다.
  공개 또는 제출 가능한 자료로 승격하려면 별도 review와 사용자 승인이 필요하다.
