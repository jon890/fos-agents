...
[매일 아침]
  ↓
  /study-topic-recommender  → data/runtime/morning-topic-recommendation.md
  ↓ (10픽 + 오늘의 3선)
  사용자가 1개 토픽 선택
  ↓
  /study-pack-writer <topic>      → sources/fos-study/<domain>/<topic>.md
  ↓ (또는)
  /interview-asset-writer <topic> → sources/fos-study/interview/<형식별 경로>/<topic>.md
  ↓
...
[정기 백그라운드 — cron]
  ...
  ↓
  /position-recommender    → data/runtime/position-recommendation.md
  ↓
...
dispatcher 폐기 완료 (plan023, ADR-031) — 모든 명령은 agent skill 직접 호출로 진입한다.
문서상 호출은 `/<skill> [args]` 형태의 의도 표현을 표준으로 쓰며, 각 실행 환경이 같은 SKILL.md 워크플로를 실행한다.
완료/실패 시 Discord 알림을 발송한다 (`bun --env-file=career-os/.env _shared/lib/notify_discord.ts` 경유, ADR-021).
...
### `/interview-prep-analyzer` (agent skill — plan017, baseline + daily 두 모드 자연어 분기)

agent skill 패턴: `/interview-prep-analyzer [args]` → SKILL.md 로드 → 현재 에이전트가 도구로 직접 처리.
...
```
  ...
  │ 에이전트 분석 → 7 섹션              │       │  - 없으면 config/study-progress.json │
...
agent skill 패턴: `/study-topic-recommender` → SKILL.md 로드 → 현재 에이전트가 도구로 직접 처리.

on-demand 실행에서는 후보 제안 JSON, runtime report, inventory markdown을 쓰고 `Bash`로 Bun 스크립트를 실행한다.
Claude CLI 호환 경로로 비대화형 실행할 때는 권한 승인 요청에서 멈추지 않도록 적절한 permission mode를 사용한다.
다른 에이전트는 자기 실행 환경의 권한 모델에 맞춰 같은 SKILL.md 워크플로를 실행한다.
...
### `study-pack <topic>` (agent skill — ai-nodes ADR-002, plan013 + ADR-033)
...
### `interview-asset <topic>` (agent skill — plan015, Q&A + master playbook 두 형식)
...
### `/interview-prep-analyzer first-round|final-round|offer-chat` (agent skill — ADR-027 + ADR-048)

agent skill 패턴: `/interview-prep-analyzer first-round` → SKILL.md 로드 → 현재 에이전트가 도구로 직접 처리.

면접 단계 분기: first-round / final-round / offer-chat.

```
호출: /interview-prep-analyzer <stage>
  ↓
...
Bash: bun career-os/scripts/interview-prep-analyzer/collect_interview_sites.ts --mode <stage>
  ...
  ↓
Read: candidate-profile.md + private/<company>/<position>/interview/prep.md
      + 수집된 sites text + 관련 fos-study 학습 문서
  ↓
에이전트 분석 → 예상 질문 / 답변 리스크 / 역질문 / 확인 필요 항목 작성
  ↓
Write: data/reports/daily/YYYY-MM-DD/interview-prep-<stage>/report.md
```

회사명·URL은 `config/mvp-target.json`의 `primary.interview.<stage>` 객체에서 읽는다.
준비 자산 정본은 `primary.data_root` 아래 `interview/prep.md`다.
...
사용자
  ...
  -> host-side processor
     question-bank-collector skill 호출
...
1. 현재 에이전트가 SKILL.md를 로드하고 도구를 직접 실행한다.
...
- 에이전트 실행 타임아웃: runner가 비-zero exit, Discord [실패] 알림. baseline은 추가로 `report.fallback.md` 생성해 부분 정보 보존.
...
dispatcher 폐기 후 `run_now.sh`는 존재하지 않음. agent skill 직접 호출이 유일한 진입점 — 우회 경로 없음.

디버깅·단발 테스트 시에도 동일한 `/<skill> [args]` 호출 의도를 사용한다.
