...
구현 전에 비교할 절감안을 정리한다.
실측과 원인 조사를 진행한 단계에서는 코드를 바꾸지 않았고 `fos-assistant` 저장소도 수정하지 않았다.
이후 사용자가 구현을 요청해 `career-os` 변경은 별도 Orca 작업으로 진행했다.
...
확인한 DB 대상에 따라 study table 상태가 달랐다.
홈서버의 `fos_blog_db`에서는 13개 table이 모두 0행이었지만,
`fos-blog` 작업본의 환경 설정이 가리킨 DB에는 같은 table과 migration이 없었다.
따라서 전환 직전에 실제 운영 대상을 다시 확인하고,
새 API와 client 전환을 검증하기 전에는 기존 table을 제거하지 않는다.
...
이 안만 적용하면 MySQL 없이도 첫 실행의 후처리 왕복을 줄일 수 있다.
최종 설계에서는 반복 cron의 분석 재사용이 필요해 안 B와 함께 적용한다.
...
HTML과 최종 답변은 `coupang-careers 상세 62건을 확인하지 못해 후보가 누락됐을 수 있다`처럼 표시한다.
...
- HTTP Backend 적용 뒤 API 호출 수, model API 호출 수와 반복 cron 입력 토큰은 아직 측정하지 못했다.
- 첫 실행과 두 번째 실행에서 실제로 줄어드는 토큰 비율은 아직 확인하지 못했다.
- 짧은 색인만 본 1차 선별이 현재 상위 후보를 놓치지 않는지 확인하지 못했다.
- background review 5회가 사용자별 비용 화면에 별도로 합산되는지 확인하지 못했다.

파일 기반 재사용과 전용 명령 뒤에도 첫 실행 입력이 크면 안 C를 적용한다.
이 순서는 `career-os` 안의 변경으로 먼저 실측하고,
