# career-os 제품 요구사항

career-os는 커리어 탐색부터 지원 준비, 면접 연습, 기술 학습까지 이어지는 개인 운영 도구다.

## 사용자 문제

채용 공고, 경력 근거, 지원 문서, 면접 준비 자료가 흩어지면 판단과 개선이 반복되지 않는다.
career-os는 외부 자료를 수집하고 사용자의 실제 이력과 대조해 다음 행동을 정할 수 있게 한다.

## 제품 약속

- 실제로 열린 개별 공고만 추천 후보로 다룬다.
- 추천과 이력서의 주장은 확인 가능한 경력과 작업 근거에 연결한다.
- 마감 여부, 중복, 명시적 제외처럼 결정적으로 판정할 수 있는 조건은 코드로 처리한다.
- 공고 적합도와 읽을거리의 최종 선별은 모델이 맥락을 보고 판단한다.
- 사용자가 보는 분석 결과는 공개 범위를 점검한 HTML 리포트로 제공한다.
- 지원서 제출, 로그인, 메시지 전송, 공개 게시처럼 외부 상태를 바꾸는 작업은 사용자 승인 뒤에 수행한다.

## 제품 영역

### 공고 탐색과 지원 판단

`position-recommender`는 등록된 채용 소스에서 공고를 수집한다.
수집 단계는 닫힌 공고와 명백한 제외 대상을 제거하고, 모델은 남은 후보 중 지원 가치가 높은 공고를 추천한다.

`job-fit-analyzer`는 선택한 역할의 지원 판단, 커리어 연결점, 면접 전략을 정리한다.
현재 지원 대상은 로컬 상태 파일 하나로 선택적으로 관리한다.

### 지원 문서와 검증

`application-package-writer`는 공고와 후보자 이력을 바탕으로 지원 패키지를 만든다.
`application-reviewer`는 제출 전 누락, 과장, 공고 요구사항 반영 여부를 판정한다.

`resume-evidence-auditor`는 이력서 주장과 업무 문서, 코드, 테스트, Git 이력을 대조한다.
`resume-evaluator`는 근거가 확인된 HTML 이력서의 내용과 표현을 채점하고 개선한다.

### 면접 준비와 답변 연습

`interview-stage-prep`은 면접 단계에 맞는 조사 항목과 준비 행동을 만든다.
기술·인성 드릴은 사용자가 먼저 답한 내용을 채점하고, 복습 상태를 다음 실행에 반영한다.

### 아침 읽을거리와 질문 은행

`study-topic-recommender`는 회사 기술 블로그와 개발 동향 소스를 수집한다.
고정 주제를 생성하지 않고, 해당 날짜에 수집된 외부 글 중 읽을 가치가 높은 자료를 선별한다.

`question-bank-collector`는 공개 가능한 일반 면접 질문을 검증해 질문 은행에 추가한다.
개인 경험에서 파생한 질문과 답변은 공개 자료와 분리한다.

## 성공 기준

- 추천 공고에 원문 URL, 활성 상태, 추천 근거가 있다.
- 종료되거나 지원 기한이 지난 공고가 추천 결과에 포함되지 않는다.
- 이력서 핵심 주장마다 확인 가능한 근거와 소유권 범위가 있다.
- 지원 패키지는 검토 판정과 다음 행동을 함께 제공한다.
- 드릴은 복습 대상을 누적하고 다음 실행에서 활용한다.
- 읽을거리 리포트는 외부 글의 제목, 출처, 요약, 추천 이유를 제공한다.
- 게시 리포트에는 공개 가능한 정보만 포함되며 URL이 실제로 열린다.

## 제품 경계

- 채용 플랫폼 계정과 지원서 제출을 자동으로 조작하지 않는다.
- 개인 경력과 지원 전략을 공개 리포트에 그대로 싣지 않는다.
- 외부 자료 없이 학습 주제나 채용 공고를 만들어내지 않는다.
- 별도 웹 제품이나 외부 데이터베이스를 필수 운영 요소로 두지 않는다.
- 실행 이력과 완료 작업을 제품 문서에 계속 쌓지 않는다.
