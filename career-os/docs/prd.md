...
수집 단계는 닫힌 공고와 개인 제외 대상을 제거한다.
남은 공고는 회사 우선순위, 분석 이력과 분석 만료 시점을 기준으로 일일 분석 대상을 고른다.
유효한 과거 분석은 재사용하고 처음 보거나 내용이 바뀌었거나 오래된 공고만 다시 분석한다.
아직 분석하지 않은 공고는 억지로 순위를 매기지 않고 분석 대기로 표시한다.
공고, 분석과 추천 실행 상태는 `career-os` HTTP Backend가 홈서버 MySQL에 저장한다.
skill과 cron은 DB에 직접 연결하지 않고 API로 필요한 공고만 조회한다.
...
이미 검증한 주장은 문구와 근거 파일이 모두 같을 때 이전 판정을 재사용하고, 달라진 주장과 근거만 다시 감사한다.
재사용 판정은 모델에 전체 이력과 근거를 전달하기 전에 코드로 수행하며, 어떤 근거가 바뀌어 다시 읽어야 하는지 출력한다.
...
`career-os`가 소유하는 `fos_career` database에 학습자료 table을 두고,
블로그 안의 공부 화면은 `career-os` HTTP API를 호출한다.
이 절은 합의한 다음 기능의 요구사항이며 현재 구현된 기능을 뜻하지 않는다.

```mermaid
flowchart LR
    Sources[기술 블로그와 YouTube] --> Collect[career-os 최근·과거 수집]
    Collect --> API[career-os API와 fos_career MySQL]
...
MySQL 접근과 상태 변경은 `career-os` Backend가 담당한다.
수집기, skill과 `fos-blog` 화면은 HTTP API로 자료와 개인 상태를 조회하고 변경한다.
...
기존 S3 release는 지원 준비 파일의 현재 계약을 유지하며, DB 백업·복구는 홈서버 운영 절차가 담당한다.
...
- 매일 분석할 공고 수는 비공개 정책의 상한을 넘지 않으며 상위 회사 우선 슬롯과 오래 기다린 공고 보장 슬롯을 함께 적용한다.
- 분석하지 않은 활성 공고와 부분 실패한 수집 소스는 추천 화면에서 구분해 확인할 수 있다.
...
- 지원 문서와 면접 자료는 파일 기반 운영을 유지한다.
  포지션과 학습자료의 수집·분석·추천 상태는 예외로 `career-os` HTTP API와 MySQL을 사용한다.
