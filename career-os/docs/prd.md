...
`job-fit-analyzer`는 선택한 역할의 지원 판단, 커리어 연결점, 면접 전략을 정리한다.
현재 지원 대상은 로컬 상태 파일 하나로 선택적으로 관리한다.
...
`application-reviewer`는 자동 상태 전이나 명시적인 독립 검토가 필요할 때만 내부 검사로 사용한다.

`resume-evidence-auditor`는 이력서 주장과 업무 문서, 코드, 테스트, Git 이력을 대조한다.
`resume-evaluator`는 근거가 확인된 HTML 이력서의 내용과 표현을 채점하고 개선한다.

### 면접 준비와 답변 연습

`interview-stage-prep`은 면접 단계에 맞는 조사 항목과 준비 행동을 만든다.
기술·인성 드릴은 사용자가 먼저 답한 내용을 채점하고, 복습 상태를 다음 실행에 반영한다.
