...
**dispatcher 폐기 완료 (plan023, ADR-031)** — 모든 진입점이 agent skill 직접 호출로 단일화됨.
문서상 표준 호출은 `/<skill> [args]` 형태의 의도 표현이며, 각 실행 환경이 같은 SKILL.md 워크플로를 실행한다.
...
| `/job-fit-analyzer` | 타깃 직무 역할 단위 핏 분석 + 부족분 갭 진단<br>입력: `config/mvp-target.json` + `config/candidate-profile.md`<br>`data/reports/job-fit-YYYY-MM-DD.md`<br>plan086 (interview-prep-analyzer 리네임 + 리포커스) | 없음 | 지원 전·시즌 시작 시 |
| `/tech-interview-drill` | 매일 기술 면접 답변 연습 + 3단계 채점 + 약점 환류<br>간격 반복 질문 선정 → 대화 답변 → pass/shallow/fail/unknown 채점<br>`data/runtime/drill-log-YYYY-MM-DD.jsonl` (누적)<br>`config/study-progress.json` weak_spots 갱신<br>plan086 신규 | 없음 | 매일 |
| `/behavioral-interview-drill` | 매일 인성 면접 답변 연습 + STAR·가치관 채점 + 약점 환류<br>tech-interview-drill과 동일 엔진, behavioral 질문 풀 적용<br>`data/runtime/drill-log-YYYY-MM-DD.jsonl` (누적)<br>plan086 신규 | 없음 | 매일 |
| `/interview-stage-prep` | 1차/최종/오퍼 단계별 실전 준비 자료 생성<br>입력: `config/mvp-target.json` + `config/candidate-profile.md`<br>`data/reports/stage-prep-YYYY-MM-DD.md`<br>plan086 신규 | 없음 | 면접 전 |
| `/interview-prep-analyzer` | ⚠️ 제거 예정 (plan086 phase-06 이후)<br>기능이 `job-fit-analyzer`와 `interview-stage-prep`으로 재편됨<br>plan017, ADR-027, ADR-048 | 없음 | — |
...
`plan029-application-agent-mvp`는 기존 agent skill들을 조립해 지원 전후 케어 루프를 만든다.
...
핵심은 agent skill을 순서대로 이어 붙이는 것이 아니라, ledger/runtime을 읽고 상태 기반 루프를 수행하는 TypeScript policy decision engine이다.

상태 루프: `state -> policy decision -> action -> validation -> state update`

MVP 원칙:

- 기존 agent skill은 tool로만 재사용한다.
...
- 수집 snapshot DB import는 LLM 추천 생성보다 먼저 수행한다.
...
- 기존 agent skill을 새 generator로 대체하지 않고 바로 연결한다.
...
- processor가 request를 읽고 career-os writable checkout에서 허용된 agent skill만 호출한다.
...
- dashboard container에서 특정 에이전트 CLI를 직접 실행하는 일.
...
- 토픽 자동 promotion에 사용자 검토 우회.
  - LLM은 제안만, 로컬 validator가 게이트 (ADR-011)
