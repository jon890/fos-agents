...
**dispatcher 폐기 완료 (plan023, ADR-031)** — 모든 진입점은 agent skill `/<skill> [args]` 계약으로 단일화됐다.
특정 CLI runner는 무인 실행용 compatibility backend로만 둔다.
...
| `/job-fit-analyzer [역할]` | 지원 의사결정(go/no-go) + 면접 전략(강점 어필·약점 방어) + 커리어 패스 정합 진단<br>자연어 역할 인자 또는 `config/mvp-target.json` fallback + `config/candidate-profile.md`<br>JSON 정본 `data/reports/job-fit-YYYY-MM-DD-<slug>.json` → md 파생<br>ADR-096 (의사결정·전략 재정의, JSON 정본) | 없음 | 지원 전·시즌 시작 시 |
