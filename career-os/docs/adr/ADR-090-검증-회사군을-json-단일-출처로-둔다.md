...
- 검증 회사군 JSON을 `references/`에서 `config/verified-company-research-targets.json`으로 옮겨 단일 출처로 둔다. 텍스트 목록의 회사를 JSON으로 흡수한다.
  - 이유 — career-os config는 사람이 큐레이션한 정책과 외부 읽을거리 수집 대상을 담고 코드가 읽는 설정이다(`config/external-reading-sources.json`과 같은 부류). 양방향(코드 discovery + LLM 주입) 소비에 맞고, `data-schema.md`가 스키마를 관리한다.
