...
career-os의 주요 자동화는 `.claude/skills/<skill>/SKILL.md`에 정리되어 있다.
...
`.claude/skills`와 `.codex/skills`에 본문을 복사하면 같은 workflow가 두 곳에서 갈라질 위험이 크다.
...
- `.codex/skills`에 SKILL.md 본문을 복사: 두 사본이 갈라져 drift가 생긴다.
- Codex에서 `claude -p` wrapper 호출: 비용 문제가 해결되지 않는다.
