...
- **tech-interview-drill / behavioral-interview-drill** (신규): 매일 면접 질문에 1문장으로 답하고 채점·약점 환류하는 드릴. 기술·인성은 물어보는 관점이 달라 별도 스킬로 두되, 질문 선정부터 약점 기록까지 공용 드릴 엔진을 사용한다.
...
- 매일 답변 연습에서 확인한 약점이 다음 질문과 외부 읽을거리 추천에 반영된다.
- weak_spots가 실제 답변 데이터로 갱신돼 정체 문제가 해소된다.
- 각 스킬이 단일 책임을 가져 사용자는 상황에 맞는 스킬만 호출한다.
- question-bank-collector는 공개 질문 수집과 누적을 계속 담당한다.
