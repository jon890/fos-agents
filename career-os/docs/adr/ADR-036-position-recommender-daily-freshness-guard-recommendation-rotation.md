...
- Status: Partially superseded by ADR-100, ADR-101, and ai-nodes ADR-020
- Date: 2026-05-23
...
- daily cron은 `scripts/position-recommender/run_daily_with_claude.sh`를 호출한다.
  ...
  - freshness check 통과 후 skill이 게시 준비용 리포트와 공개 가능한 요약을 반환한다.
  - 외부 게시를 명시적으로 요청하면 루트 `report-publisher`가 공개 범위를 점검하고 검증된 URL을 반환한다.
...
- skill 실행과 외부 전달의 실패 경계를 분리한다.
- stale 재전송은 막되, 닫히지 않은 좋은 공고의 반복 추천은 지원 액션을 촉구하는 신호로 유지할 수 있다.
- 실제 추천 작업과 자체 검증은 native skill이 수행한다.
