...
- Status: Partially superseded by ai-nodes ADR-011
- Date: 2026-06-09
...
- 현재 skill은 추천 3개와 action snapshot을 생성한다.
- 외부 전달 계층은 snapshot을 사용해 버튼을 구성할 수 있다.
- 버튼은 `1번 초안 생성`, `2번 초안 생성`, `3번 초안 생성`, `오늘은 넘김`으로 둔다.
- 버튼 callback 매핑은 `state/study-topic-actions/YYYY-MM-DD.json`과 `latest.json`에 저장한다.
- `career.study-pack.create:*`는 study-pack 초안 생성 요청이다.
  공개 최종화나 `[초안]` 제거가 아니다.
- `career.study-pack.skip:*`는 그날 추천을 넘긴 기록이다.
  해당 topic을 영구 제외하지 않는다.
- 버튼 유효시간은 외부 전달 계층이 정한다.
