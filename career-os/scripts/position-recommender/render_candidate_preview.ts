...
export function renderCandidatePreviewHtml(
  ...
  const recommendedRows = recommendationRows(run);
  const recommended = options.candidatePool ? recommendedRows : applyLimit(recommendedRows, limit);
  ...
  const remaining = recommended.filter((row) => row.tier !== "강력 추천");
...
</style></head><body><main class="shell"><header class="masthead"><div class="masthead-top"><div><p class="eyebrow">POSITION RECOMMENDATION</p><div class="title-line"><h1>${escapeHtml(title)}</h1><span class="recommendation-count"><strong>${strong.length}</strong><span>강력 추천</span></span></div></div><div class="meta-group"><p class="compact-meta"><span>${escapeHtml(run.reportDate.replace(/-/g, "."))} 생성</span><span>${escapeHtml(collected.short)} 수집</span></p><details class="provenance" data-run-id="${escapeHtml(collectionRunId)}"><summary>수집 정보</summary><div><span>${escapeHtml(collected.full)} 수집</span><span>${escapeHtml(runType)} 실행</span></div></details></div></div><section class="conclusion" aria-label="오늘의 추천 요약"><ul>${conclusions}</ul></section></header>
<div class="section-head"><h2>강력 추천</h2><span class="count">지원 판단에 필요한 정보를 한 화면에 압축</span></div><section class="priority-grid">${heroCards}</section>
<div class="section-head"><h2>다음 판단</h2><span class="count">${remaining.length}건</span></div><section class="board">${boardRows}</section>
