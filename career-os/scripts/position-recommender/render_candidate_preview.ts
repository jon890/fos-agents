...
/** 상위 카드에 축별 판정을 놓아, 종합 방향이 어느 축에서 왔는지 바로 보이게 한다. */
function renderUpsideAxes(row: PreviewRow): string {
  if (!row.upsideAxes || row.upsideAxes.length === 0) return "";
  const items = row.upsideAxes
    .map(
      (axis) =>
        `<li><span class="axis-name">${escapeHtml(axis.axis)}</span>` +
        `<span class="axis-dir dir-${directionClass(axis.direction)}">${escapeHtml(axis.direction)}</span>` +
        `<span class="axis-reason">${escapeHtml(axis.reason)}</span></li>`,
    )
    .join("");
  return `<ul class="upside-axes">${items}</ul>`;
}
...
function upsideBadge(row: PreviewRow): string {
  if (!row.upsideDirection) return "";
  const cls = directionClass(row.upsideDirection);
  return `<span class="upside upside-${cls}" title="현재 직장 대비 업사이드">${escapeHtml(row.upsideDirection)}</span>`;
}

function renderCandidateArchive(rows: PreviewRow[]): string {
  const candidates = rows.map((row) => `<article class="candidate-row" data-search="${escapeHtml([row.company, row.title, row.why, row.upsideDirection ?? "", ...row.keywords].join(" ").toLowerCase())}">
