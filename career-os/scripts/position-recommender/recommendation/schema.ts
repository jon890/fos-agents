...
export const CompanyFindingTopic = z.enum([
  "domain-growth",
  "role-ownership",
  "team-growth",
...
export const CompanyAssessmentFinding = z
  .object({
    kind: z.enum(["fact", "inference"]),
    topic: CompanyFindingTopic,
...
export const CompanyAssessment = z
  .object({
    summary: z.string().trim().min(1),
    confidence: z.enum(["high", "medium", "low"]),
    findings: z.array(CompanyAssessmentFinding).min(1),
  })
  .strict();

/** 강력/도전 추천 포지션 1건. 회사 평가는 근거가 있는 자유로운 finding으로 구성한다. */
...
export const RecommendationRun = z
  .object({
    ...
    evaluatedCandidateIds: z.array(z.string().min(1)).min(1),
  ...
  .superRefine((run, ctx) => {
    ...
    run.evaluatedCandidateIds.forEach((candidateId, index) => {
      if (evaluatedIds.has(candidateId)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["evaluatedCandidateIds", index],
          message: `검토한 후보 ID가 중복됐다: ${candidateId}`,
        });
      }
      evaluatedIds.add(candidateId);
    });

    const selectedIds = new Set<string>();
    const ranks = new Set<number>();
    [...run.tiers.strong, ...run.tiers.stretch].forEach((item, index) => {
      if (selectedIds.has(item.candidateId)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["tiers", index, "candidateId"],
          message: `추천 티어에 중복 공고 ID가 있다: ${item.candidateId}`,
        });
      }
      if (ranks.has(item.rank)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["tiers", index, "rank"],
          message: `추천 티어에 중복 순위가 있다: ${item.rank}`,
        });
      }
    ...
    if (orderedRanks.some((rank, index) => rank !== index + 1)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["tiers"],
        message: "추천 순위는 1부터 중복 없이 이어져야 한다",
      });
    }
