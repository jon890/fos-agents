...
- adapter output은 공통 `validator.ts`를 다시 통과해야 한다.
...
- Coupang처럼 공식 detail/listing HTML이 fetch에서 차단되는 회사도 공식 sitemap처럼 접근 가능한 direct job URL source를 별도 adapter로 사용한다.
