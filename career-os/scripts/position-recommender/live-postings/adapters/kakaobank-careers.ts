...
export function parseKakaoBankPosting(notice: RecruitNotice, html: string): Posting | null {
  ...
  return {
    ...
    title,
    url,
    identityHash: `kakaobank-careers:${notice.recruitNoticeSn}`,
    linkType: "direct_posting",
    postingStatus: "open",
    activeEvidence: `KakaoBank official recruits API lists notice ${notice.recruitNoticeSn} with an active receipt window`,
    openedAt: notice.receiveStartDatetime,
    ...closeWindow(notice.receiveEndDatetime),
    category: notice.recruitClassName,
    summary: "카카오뱅크 공식 영입 공고",
    ...
    mainTasks: section(
      text,
      "(?:담당할\\s*업무|주요\\s*업무)",
      ["필수\\s*경험", "자격\\s*요건", "우대사항"],
      650,
    ),
    requirements: section(
      text,
      "(?:필수\\s*경험과\\s*역량|자격\\s*요건)",
      ["우대사항", "근무\\s*관련\\s*정보"],
      650,
    ),
...
export const kakaobankCareersAdapter: SourceAdapter = {
  id: "kakaobank-careers",
  name: "kakaobank-careers",
  async collect(): Promise<AdapterCollectionResult> {
    ...
    const list =
      listing.ok && listing.data && typeof listing.data === "object"
        ? ((listing.data as { list?: RecruitNotice[] }).list ?? [])
        : [];
    ...
    for (const notice of list) {
      try {
        ...
        if (!detail.ok) {
          failedCount++;
          errors.push(`kakaobank-careers detail ${notice.recruitNoticeSn}: HTTP ${detail.status}`);
          continue;
        }
        ...
        failedCount++;
        errors.push(`kakaobank-careers detail fetch failed: ${error}`);
      }
    }
    return {
      postings,
      diagnostics: {
        ...
        message: `kakaobank-careers diagnostics: detail_candidates=${list.length}, accepted=${postings.length}, skipped=${skippedCount}, failed=${failedCount}`,
      },
      errors,
    };
  },
