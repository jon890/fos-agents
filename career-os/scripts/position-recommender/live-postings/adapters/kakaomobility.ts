...
function postingFromDetail(url: string, html: string): Posting | null {
  ...
  return {
    ...
    mainTasks: cleanDetail(
      text.match(/(?:합류하게\s*되면\s*이런\s*일을\s*하게\s*됩니다|하게\s*될\s*일|담당\s*업무|업무\s*내용)([\s\S]*?)(?:이런\s*분을\s*찾습니다|이런\s*분이\s*필요해요|지원\s*자격|지원자격|자격\s*요건|자격요건|필요\s*역량|필요역량|이런\s*분이면\s*더\s*좋습니다|우대\s*사항|우대사항)/)?.[1] ?? text, 650),
    requirements: cleanDetail(
      text.match(/(?:이런\s*분을\s*찾습니다|이런\s*분이\s*필요해요|지원\s*자격|지원자격|자격\s*요건|자격요건|필요\s*역량|필요역량)([\s\S]*?)(?:이런\s*분이면\s*더\s*좋습니다|우대\s*사항|우대사항|우대\s*역량|지원\s*안내|합류\s*여정|전형\s*안내)/)?.[1] ?? "", 650),
    preferred: cleanDetail(
      text.match(/(?:이런\s*분이면\s*더\s*좋습니다|우대\s*사항|우대사항|우대\s*역량)([\s\S]*?)(?:지원\s*안내|합류\s*여정|근무\s*조건|근무조건|전형\s*안내|처우\s*안내)/)?.[1] ?? "", 500),
  };
}
