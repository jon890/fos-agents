...
function postingFromDetail(url: string, html: string): Posting | null {
  ...
  return {
    ...
    requirements: cleanDetail(
      text.match(/(?:필요\s*역량\/경험|이런\s*분을\s*찾습니다|이런\s*분이\s*필요해요|지원\s*자격|지원자격|자격\s*요건|자격요건|필요\s*역량|필요역량)([\s\S]*?)(?:선호\s*역량\/경험|이런\s*분이면\s*더\s*좋습니다|우대\s*사항|우대사항|우대\s*역량|지원\s*안내|합류\s*여정|전형\s*안내)/)?.[1] ?? text, 650),
    preferred: cleanDetail(
      text.match(/(?:선호\s*역량\/경험|이런\s*분이면\s*더\s*좋습니다|우대\s*사항|우대사항|우대\s*역량)([\s\S]*?)(?:지원\s*안내|합류\s*여정|근무\s*조건|근무조건|전형\s*안내|처우\s*안내|유의사항|전형\s*절차)/)?.[1] ?? "", 500),
  };
}
