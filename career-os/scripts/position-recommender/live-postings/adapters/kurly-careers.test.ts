...
test("컬리 공식 목록에서 개별 공고 URL만 찾는다", () => {
  expect(
    extractKurlyDetailUrls(
      '<a href="/ko/o/198009">공고</a><a href="https://kurly.career.greetinghr.com/ko/o/198010">공고</a>',
    ),
  ).toEqual([
...
test("공식 개별 공고의 적용 가능 상태와 JD를 Posting으로 정규화한다", () => {
  ...
  expect(posting?.skills).toEqual(
    expect.arrayContaining(["Java", "Kotlin", "Spring", "AWS", "Kafka"]),
  );
});
...
test("본문의 우연한 키워드만으로 비개발 직무를 수집하지 않는다", () => {
  const mdHtml = detailHtml
    ...
    .replace(
      "Payment 및 Billing 시스템을 Java와 Kotlin, Spring으로 설계하고 운영합니다.",
      "캠페인과 상품 운영을 담당합니다.",
    );
