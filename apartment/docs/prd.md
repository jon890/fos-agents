...
| 번호 | 명령 | 산출물 | 빈도 |
|---|---|---|---|
| 1 | `apartment-daily-report` (skill, `scripts/run_report.sh`) | `data/YYYY-MM-DD/{report.md, raw-search.json, summary.json, claude.result.json}` + Discord 완료 알림 | 매일 08:00 cron |
| 2 | `apartment-interior-reference-digest` (skill, `scripts/run_digest.sh`) | `data/interior-reference-digest/YYYY-MM-DD/report.md` + Discord 요약 (3 결정 질문) | 매일 |
...
`skills/apartment-daily-report/scripts/notify_discord.sh` 직접 호출:
- 시작 알림
- 완료 알림 (소요 시간 포함)
- 실패 알림
