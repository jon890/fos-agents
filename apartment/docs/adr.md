...
## ADR-010 — apartment-daily-report native skill 전환 (외부 subprocess 패턴 폐기)
...
- career-os가 ADR-002로 native skill 직접 호출로 전환하며 폐기한 패턴이다.
  apartment도 interior-reference-digest는 plan007에서 이미 native로 전환했고, daily-report만 옛 패턴이 잔존한다.
- `track_task.sh`가 남기는 `task-runs.jsonl` / `token-usage.jsonl`은 읽는 소비자가 없다(write-only 죽은 계측).

### 결정

`apartment-daily-report`를 native skill 직접 호출로 전환한다.

- 수집·정규화 TS 헬퍼(`collect_sources.ts`, `normalize_results.ts`, `_lib/load_target_meta.ts`)는 유지한다.
  네이버 API·agent-browser 수집은 결정론적이라 Claude로 재구현하지 않는다.
  native SKILL.md가 `bun` Bash로 호출한다.
- `claude --output-format json` + `extract_claude_result.ts` 추출 단계는 폐기한다.
  Claude가 summary.json을 Read하고 report.md를 직접 Write한다.
- `track_task.sh` self-wrap(TRACK_TASK_WRAPPED 가드)을 제거한다.
- 진입점은 interior의 `run_with_claude.sh` thin wrapper 패턴을 따른다.
  `claude -p "/apartment-daily-report"` 호출 + Discord 시작/실패 알림.
  완료/요약 알림은 native skill이 SKILL.md 지시로 담당.

연쇄 폐기 자산(`track_task.sh`, `update_artifacts.py`, `extract_claude_result.ts` 파일 자체)은 본 plan 범위가 아니다.
stock-investment 전환까지 끝나 호출처가 0이 된 뒤 후속 ai-nodes 모노레포 정리 plan에서 제거한다.
본 plan은 apartment daily-report의 *호출만* 끊는다.

거절한 대안:

- 옛 subprocess 패턴 유지 — career-os가 입증한 drift·복잡도를 영구화.
- 수집도 Claude WebFetch로 — 네이버 API·agent-browser 수집 로직을 버려 데이터 품질 저하.
- track_task/extract 파일을 본 plan에서 즉시 전면 삭제 — stock-investment가 아직 사용, 워크스페이스 격리 위반.

### 결과

- daily-report가 interior와 동일한 native 패턴으로 수렴 — apartment 내 일관성.
- `claude.result.json` / `report.fallback.md` 산출물 폐기.
- apartment에서 `track_task.sh` / `extract_claude_result.ts` 호출처가 0이 된다(stock-investment만 잔존) — 후속 모노레포 폐기 plan의 선결 조건.
- (B) native 마이그레이션의 파일럿 — 검증 후 stock-investment 3 skill에 패턴 복제.
- 단점: native skill이 데이터 파이프라인 오케스트레이션까지 책임 — 결정론적 수집 실패 처리를 SKILL.md가 명시해야 함.

**적용**: `apartment/.claude/skills/apartment-daily-report/SKILL.md`(native 동작 명세 재작성) + `scripts/apartment-daily-report/run_with_claude.sh`(신규 thin wrapper) + `run_report.sh` 폐기 + references 옛 프롬프트 정리. `apartment/tasks/plan010-daily-report-native` phase 참조.

---

## ADR-011 — 인테리어 의사결정 HTML 뷰 동반 산출
...
인테리어 결정 문서는 Markdown 원본으로 누적되고 있지만, 결정된 항목과 남은 항목을 대화에서 다시 볼 때 가독성이 떨어진다.
사용자는 앞으로 의사결정한 부분과 남은 부분을 HTML로 만들어 보기 좋게 확인하길 원한다.

### 결정

인테리어 의사결정/남은 결정 항목을 사용자에게 보여줄 때는 Markdown 원본 갱신과 함께 HTML 뷰를 동반 산출한다.
...
결정 관리는 Markdown 원본의 추적성을 유지하면서, 사용자 확인용 화면은 HTML로 더 읽기 쉽게 제공한다.
