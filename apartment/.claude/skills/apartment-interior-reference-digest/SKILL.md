...
의사결정/남은 결정 항목을 사용자에게 보여주는 리포트에는 같은 폴더에 `decision-view.html`을 함께 저장한다.

- HTML은 사람이 읽는 뷰이고, 결정의 단일 출처는 계속 `docs/interior/*.md`다.
- HTML에는 확정 결정, 남은 결정, 현장 확인 항목을 구분해 표시한다.
- 카드/섹션은 짧고 스캔하기 좋게 구성한다.
- 사용자가 "보여줘"처럼 열람을 요청하면 Discord에는 핵심 요약과 함께 `decision-view.html` 파일을 첨부한다. 경로만 안내하는 것으로 끝내지 않는다.
- cron/자동 실행처럼 사용자가 직접 열람을 요청하지 않은 경우에는 핵심 요약과 HTML 파일 경로만 보낸다.
- 새 결정이 들어오면 먼저 Markdown 원본 문서를 갱신한 뒤 HTML을 다시 만든다.
...
- 레퍼런스 누적 여부
- 의사결정 문서 반영 후보
- HTML 의사결정 뷰: `decision-view.html` 경로와 포함 범위
