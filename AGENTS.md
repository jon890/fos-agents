...
PR은 merge commit을 만드는 방식으로 머지한다.
squash와 rebase 머지는 사용하지 않는다.

```bash
gh pr merge <번호> --merge --delete-branch
```

squash는 브랜치의 커밋을 하나로 합치면서 원래 해시를 버린다.
그래서 같은 브랜치에 얹었지만 원격에 올리지 않은 로컬 커밋이 머지 대상에서 빠지고,
브랜치를 지우는 순간 어디에도 속하지 않게 된다.
실측으로 커밋 4개를 이 방식으로 잃고 reflog에서 되살렸다.

merge commit은 브랜치의 커밋을 그대로 남기므로 어느 변경이 어느 작업에서 왔는지 이력에 남는다.
