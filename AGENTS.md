...
- `.claude/skills/` — 저장소 전역 Claude Code 스킬 (`agent-browser`, `planning`, `plan-and-build`, `workspace-audit`, `docs-check`). `docs-check`는 ai-nodes 5문서 + ADR 건전성 감사 스킬. `.codex/skills/`는 같은 본체를 가리키는 심링크 (ADR-013/014).
...
- 운영 호환 진입점: 무인 runner 또는 thin wrapper `run_with_claude.sh`.
...
두 skill 모두 agent skill 직접 호출 (ADR-010).
수집·정규화 TS 헬퍼는 skill이 `bun` Bash로 호출하고, 현재 에이전트가 summary.json을 읽어 report.md를 직접 작성한다.

### 3-2. career-os

현재 표준은 **agent skill 직접 호출**이다 (ai-nodes ADR-002/013, plan013~023). dispatcher 흐름은 plan023에서 완전 폐기 (ADR-031).

agent skill 진입점 12개:

```text
cd career-os
/position-recommender [자연어 컨텍스트] [채용공고 file]
/application-package-writer <posting-path>
...
/job-fit-analyzer
/interview-stage-prep [first-round|final-round|offer]
...
- **모든 워크스페이스**: agent skill 직접 호출 — ADR-011/013 단일 패턴.
  ...
  - 운영 runner는 특정 에이전트 CLI compatibility backend를 계속 쓸 수 있다.
...
- 비대화형 planning CLI 호출은 기본 사용하지 않는다. 계획은 반박, 보류, 범위 조정, 승인 절차 논의가 필요하므로 비대화형 생성에 맡기지 않는다.
...
`.claude/skills/planning` 구조로 Codex와 사용자가 plan을 대화형으로 작성하고, 합의 후 task 파일로 고정한다. 구현은 Claude 비대화형 실행 또는 `.claude/skills/plan-and-build` 자동 실행(`run-phases.py`)으로 진행한다.
...
- 설치: brain repo의 `skills/brain-*`를 `~/.claude/skills/`에 symlink. 에이전트 세션이 전역 skill로 발견.
