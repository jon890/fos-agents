...
인테리어 추천은 Claude native skill 직접 호출 경계가 중요하므로 `run_with_claude.sh`를 운영 진입점으로 사용하며, native skill(`/apartment-interior-reference-digest`)이 전체 workflow를 수행한다 (apartment ADR-010 — 옛 `run_digest.sh` scaffold 폐기).
