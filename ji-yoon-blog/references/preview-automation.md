...
사진을 고른 뒤 `문서 너비`를 적용하면 `se-component-content-fit` 클래스가 붙는다.
...
| 제목이 비었다 | 안내 문구 `제목` 만 읽혔다 | 화면을 막 열면 편집기가 스스로 본문에 초점을 옮긴다. 제목 글자가 본문에 들어갔다가 본문을 지울 때 사라진다. 누른 뒤 커서가 제목 안에 있는지 보고 넣는다 |
