...
| 홈서버의 Hermes 컨테이너 | 홈서버와 같다. 저장소 옆의 환경 파일에서 S3 값을 읽는다 | SSH 키 |
| 맥북 | 내려받은 사진, 초안 | S3 credential |

홈서버에는 이 저장소의 체크아웃이 하나 있다.
운영과 사람의 작업이 그 하나를 함께 쓴다. Hermes 컨테이너가 보는 것도 그것이다.

**코드 경로를 따로 떼어 내지 못한다.**
그 트리에 코드만 있는 것이 아니라 워크스페이스들이 쓰는 운영 데이터가 함께 있다.
떼어 내면 그 데이터를 쓰는 쪽이 자기 것을 잃고,
복사해 나누면 같은 데이터의 소스가 둘이 되어 더 나빠진다.

그래서 사람이 그 체크아웃의 브랜치를 바꾸면 운영이 함께 흔들린다.
**그 체크아웃을 `main` 에 맞추는 수단과 갱신 절차는 홈서버 운영 저장소가 소유한다.**
여기 적지 않는 이유는 둘이다.
이 저장소가 공개라 실행 환경 식별자를 담지 않고,
같은 절차를 두 곳에 적으면 한쪽만 고쳐진다.

갱신을 `git pull` 하나로 끝내지 않는다.
그 트리를 상주 서비스가 실행 중에 읽으므로 서비스를 먼저 내리고 갱신한 뒤 올린다.
실측으로 그 순서를 지키지 않아 갱신 도중 서비스가 정상으로 내려가지 못했고,
그때 네이버 로그인이 끊겼다.
...
대신 저장소와 같은 상위 디렉터리의 `apps/ji-yoon-blog/config/host.env`에서 S3 값을 읽는다.
Hermes가 profile마다 `HOME`을 바꿔도 같은 파일을 찾는다.
환경 파일이 다른 자리에 있으면 `JI_YOON_BLOG_HOST_ENV_PATH`로 절대 경로를 지정한다.
워크스페이스 `.env`가 있으면 그 파일을 먼저 읽는다.
