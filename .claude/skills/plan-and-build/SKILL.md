...
`<workspace>/docs/` 하위 문서를 읽어 워크스페이스 기획·아키텍처·결정 의도를 파악한다. 필요 시 여러 Explore 에이전트를 병렬로 사용한다.

참조 문서:

- `<workspace>/docs/README.md` — 문서별 책임과 작성 규칙 (있을 때 먼저 확인)
- `<workspace>/docs/prd.md` — 제품 가치, 사용자 문제, skill 자산, 다음 가치 방향
...
- `<workspace>/docs/prd.md` — 제품 가치·범위·우선순위 변경 반영
...
- 산출물 (`<workspace>/data/...` 또는 `<workspace>/docs/...` 변경)을 문서 책임과 대조
