...
- **필요한 정보만 담은 문서**: 의사결정 이유와 실행 계약은 보존한다.
  중복된 설명과 코드로 확인 가능한 구현 상세만 제거한다.
...
사용자가 `/planning <기능 설명>`을 호출하면 아래 8단계(+ 선택적 0단계 보정)를 **순차적으로** 진행. 각 단계는 사용자의 확인을 거친 후 다음 단계로. 규모가 작은 기능은 1+2를 합치거나 3을 생략할 수 있다.
