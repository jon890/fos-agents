...
하나라도 걸리면 사용자에게 알리고 **실행 차단** — main 에 task 부재 시 원격 작업 브랜치 자동 스캔 필수.
...
| **executor**      | `<workspace>-executor`                                                    | sonnet    | phase 순차 실행, 코드 수정 (커밋 제외), `bypassPermissions`                                                                                                                                                                                                                                                     |
| **code-reviewer** | `oh-my-claudecode:code-reviewer`                                          | sonnet    | 코드 품질 검사 (PASS/FIX_NEEDED), AI slop/금지사항 탐지                                                                                                                                                                                                                                                         |
| **docs-verifier** | `<workspace>-docs-verifier`                                               | sonnet    | 코드↔docs 정합성 검증 (PASS/UPDATE_NEEDED/VIOLATION). 전용 agent `.claude/agents/<workspace>-{executor,docs-verifier}.md` 가 도메인 지식의 단일 소스. SKILL prompt 는 호출 인자 + 직전 phase 학습 인계만.                                                                                                       |

executor·docs-verifier 는 **실행 워크스페이스명을 prefix 로 한 전용 agent**(`.claude/agents/<workspace>-{executor,docs-verifier}.md`)를 쓴다. 예: career-os 작업이면 `career-os-executor`, `career-os-docs-verifier`.
...
같은 agent 정의(`<workspace>-executor` 등)를 부르면 결과가 같아 보여 우회하기 쉽지만, 그러면 가시적 협업·SendMessage 재시도 사이클·아래 정적 검증이 모두 빠진다.
...
docs-verifier spawn 지시: **`.claude/agents/<workspace>-docs-verifier.md` 의 6축(부패·과대화·추론성·중복·자명성·가독성) + planning 8단계 D항 영향 표를 단일 소스로 검증**한다. agent 정의가 도메인 검증 항목 전체를 보유하므로 SKILL 은 위임만 한다.
...
## 프로젝트 환경 가정 (워크스페이스별 변형)

환경 가정 항목(패키지 매니저·통합 검증 명령·빌드/테스트·setup·코드 규칙 권위)은 `variants/<workspace>.md` 가 채운다.
executor·code-reviewer 프롬프트 전달 시 해당 variant 를 참조 인용한다.
