...
executor 는 `docu-parser-executor` agent definition 이 도메인 규칙의 단일 소스 — SKILL 의 spawn prompt 는 호출 인자 + 직전 phase 학습 인계만 담는다.

### default 흐름 (phase 별 spawn-shutdown 사이클 — 4 phase 이상)

매 phase 마다 새 executor 를 spawn 해서 컨텍스트 격리 + phase model 정책을 적용한다.

1. team-lead 가 `Agent({subagent_type: "docu-parser-executor", team_name: "plan{N}", name: "executor-p{N}", model: "sonnet", run_in_background: true, mode: "bypassPermissions", prompt: <표준 spawn prompt>})` 로 spawn
...
**환경 함정**: docu-parser-executor agent definition 의 <Domain_Rules> 참조.
...
**spawn prompt 작성 가이드**:
- 환경 함정 (zoxide / .venv symlink / DYLD path) 은 agent definition `<Domain_Rules>` 가 단일 소스 — 반복 금지
