...
1. **main 의 index.json status**: `tasks/{plan}/index.json` 의 `status` 확인
   ...
   - `TASK_MISSING` → **차단 전 원격 dooray 브랜치 자동 스캔 필수** (아래 1-A). main 에 task 파일이 없어도 작업 브랜치에는 있는 정상 상태일 가능성 높음.
   - `completed` → 추가 검증 필요 (PR 미머지 상태에서 `completed` 가 main 에 들어간 사고 가능성. 2·3번 결과로 판정)
   - `pending` / `in_progress` → 다음 검증으로

   **1-A. 원격 dooray 브랜치 자동 스캔 (필수 — main 에 task 부재 시)**:

   `TASK_MISSING` 인 경우 곧바로 `/planning` 으로 분기하기 전에 원격 작업 브랜치 후보를 탐색한다. CLAUDE.md 컨벤션상 task 파일은 dooray 브랜치에 먼저 생성되고 PR 머지 전까지 main 에 없는 게 정상.

   ```bash
   git fetch origin --quiet
   # 모든 dooray 작업 브랜치에서 같은 plan 이름 디렉터리 존재 여부 검색
   for ref in $(git ls-remote --heads origin 'feature/AI-TF-VectorSearch/*' | awk '{print $2}' | sed 's#refs/heads/##'); do
   ...
   사용자가 plan 번호만 알려줘도 dooray 브랜치까지 자동 식별되므로 매번 `/build-with-teams plan003 @AI-TF-VectorSearch/392` 형태로 브랜치 번호를 같이 적을 필요 없음.
...
옵션 A 는 origin/main 기반 신규 분기가 아니라 **오래된 dooray 브랜치를 그대로 이어쓴다**.
그 브랜치의 base 가 origin/main 보다 크게 뒤처져 있으면 PR 머지 충돌 + **신규 CI 단계(ruff 등) 미적용** 위험이 있다.
plan032 (브랜치 473) 는 origin/main 보다 30 commit 뒤처져 ruff(ADR-022) 도입분이 worktree 에 없었고, PR 직전 origin/main merge 로 해소했다.
...
- **0~소수**: 그대로 진행.
- **수십 commit 뒤처짐 + 그 사이 신규 CI(ruff·lint) 도입**: PR 직전 worktree 에서 `git merge --no-ff origin/main` → 충돌 해소 → **신규 CI 도구를 로컬 설치 후 통과 확인**(예: `ruff check .`) → 전체 테스트 재실행. merge commit 도 같은 PR 에 포함한다.
...
VIRTUAL_ENV=$(pwd)/.venv uv venv .venv --clear --python 3.11 && \
  VIRTUAL_ENV=$(pwd)/.venv uv pip install -r requirements.txt && \
  VIRTUAL_ENV=$(pwd)/.venv uv pip install -e ./nhn_ocr_plugin -e ./docling-paddle-plugin
```
...
- worktree 안 `.venv/` (수백 MB) 도 함께 제거되어 디스크 회수.
