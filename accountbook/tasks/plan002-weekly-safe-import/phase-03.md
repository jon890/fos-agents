...
`accountbook/scripts/accountbook-weekly-import/weekly_pipeline.test.ts`에서 비식별 PNG·manifest·validated fixture와 fetch stub을 사용해 신규 scan부터 전체 검증 preflight, 정책 승인, approved JSON API payload, submitted finalize까지 연결한다.
두 이미지에 같은 선택 날짜가 있으면 두 항목 모두 `needs_review`이고 POST가 0회인지 검증한다.
설명 `medium`, 기존 동일 거래와 불명확한 POST도 새 POST 없이 `needs_review`로 끝나는지 검증한다.
