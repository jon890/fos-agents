...
3. runtime이 이미지를 원본 해상도로 검사할 수 있는지 확인한다.
   지원하지 않으면 `OCR_UNAVAILABLE`로 중단하며 외부 OCR 서비스에 임의로 업로드하지 않는다.
4. 다음 script로 파일명, 해시, 생성 시각과 크기를 얻는다.
...
실행 경로를 만들기 전과 private JSON을 쓴 직후 다음 script를 실행한다.
이 script는 private 전체 디렉터리를 `0700`, 파일을 `0600`으로 강제한다.
...
사용자가 batch 내용을 확인하고 등록을 명시한 다음 턴에만 실행한다.
...
등록 script가 `needs_review`를 반환하면 자동으로 다시 POST하지 않는다.
성공한 수입·지출 수, 복구된 수와 검토가 필요한 수를 보고한다.

## 완료 조건

- 선택한 날짜의 상세 합계와 화면 일별 합계가 일치한다.
- 사용자가 확인한 batch ID와 승인 파일의 batch ID가 같다.
- API가 반환한 remote UUID가 private 전송 상태에 기록됐다.
- skill이 새로 만든 이미지 사본과 거래 후보는 `accountbook/private/` 밖에 남지 않는다.
