...
description: 주간 입력함의 토스 소비 화면 PNG와 보조 정보 파일을 사용자 확인 없이 처리하고, weekly-safe-v1 정책을 통과한 후보만 개인 accountbook API에 등록한다. `/accountbook-weekly-import`, "토스 캡처 주간 자동 등록", "가계부 주간 가져오기"처럼 입력함 기반 자동 처리를 요청할 때 사용한다. iPhone 업로드 주소, 예약 실행 설정, 알림 연동에는 사용하지 않는다.
---

# 가계부 주간 자동 가져오기

주간 입력함의 신규 토스 화면을 처리하고, 안전 정책을 통과한 항목만 accountbook에 등록한다.
...
특정 에이전트 명령줄 도구, 예약 실행기, 메시지 채널과 실행 환경 API는 직접 호출하지 않는다.
실행 환경이 이미지 인식을 지원하지 않으면 `OCR_UNAVAILABLE`로 중단하고 외부 OCR 서비스에 임의로 올리지 않는다.
...
- 작업 목록과 실행 계획의 `runId`가 다르면 API 호출 전에 중단한다.
- 현재 `processing`인 작업 항목과 실행 계획의 해시 집합이 다르거나 중복 해시가 있으면 API 호출 전에 중단한다.
- 상태가 `processing`인 항목만 처리한다.
- 모든 `validated.json`을 먼저 검증한다.
- 모든 이미지의 선택 날짜를 상태 파일에 기록한다.
- 이미지 간 날짜 충돌을 사전 점검한다.
- 날짜 충돌 항목은 `needs_review`로 처리하고 POST하지 않는다.
- `weekly-safe-v1` 정책을 통과한 항목만 `approved.json`으로 등록한다.
- 등록 결과를 안정 코드로 분류해 처리를 마친다.
- `finally`에서 같은 `runId`의 잠금 해제를 시도한다.
...
- 네트워크 단절, 시간 초과, 연결 종료처럼 POST 결과가 불명확한 실패: 처리를 확정하지 않고 `processing`을 유지한다.
  재실행은 비공개 전송 상태와 기존 거래 조회로만 복구한다.

## POST 안전 규칙

- 이미지 간 날짜 충돌이 있으면 관련 item은 모두 POST하지 않는다.
- `weekly-policy.json`을 등록 입력으로 쓰지 않는다.
- `submit_import.ts`가 기존 동일 거래를 발견하면 다시 POST하지 않는다.
- POST 결과가 불명확했던 후보는 재실행에서 기존 거래 조회로만 복구한다.
  복구가 모호하면 재POST하지 않고 검토 대상으로 둔다.
- POST 결과가 불명확한 직후에는 입력함 상태를 임의로 종료 상태로 바꾸지 않는다.
  다음 실행에서 비공개 전송 상태와 기존 거래 조회 결과를 기준으로 처리한다.
...
- 모든 작업 항목이 `submitted`, `needs_review`, `failed` 중 하나로 처리됐거나, 불명확한 POST 때문에 `processing` 상태로 남긴 이유가 있다.
- 자동 등록된 항목은 `weekly-safe-v1` 정책을 통과한 `approved.json`만 등록에 사용했다.
- 날짜 충돌 항목과 정책 차단 항목은 POST 0회로 끝났다.
- 같은 `runId`로 주간 실행 잠금 해제를 시도했다.
