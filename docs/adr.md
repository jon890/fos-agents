...
| ADR-012 | OpenClaw HUD는 pinned 상태 메시지 + 이벤트 기반 갱신 | Accepted | HUD 용어를 pinned 상태 메시지로 고정하고 작업 이벤트마다 갱신, 30분 stale 감지 |
...
## ADR-012 — OpenClaw HUD는 pinned 상태 메시지 + 이벤트 기반 갱신
...
또 이전 런타임 state root가 남아 있어 같은 session id가 다른 state file을 볼 가능성이 있었다.
HUD는 visible 운영 표면이므로 상태가 stale이면 오히려 사용자의 신뢰를 깎는다.
...
runtime state root는 canonical `openclaw-orchestrator/state/task-hud/` 하나만 사용한다.
legacy state root는 읽기 후보로 쓰지 않는다.
정리 전까지 legacy state는 마이그레이션 참고용으로만 둔다.

stale 기준은 30분이다.
마지막 update 이후 30분이 지나면 HUD가 stale 상태를 표시하거나 stale check가 경고를 남긴다.
...
- 전역 HUD 메시지 하나로 모든 채널을 덮어쓰기.
  - channel/session 경계가 섞인다.
...
이 호출 누락을 줄이기 위해 helper wrapper와 phase 검증을 plan050에서 추가한다.

### 적용

- HUD 정책: `openclaw-orchestrator/AGENTS.md`의 OpenClaw HUD Runtime Policy.
- helper: `_shared/lib/task_hud.ts`.
- wrapper: `openclaw-orchestrator/scripts/task-hud/`.
- 구현 task: `openclaw-orchestrator/tasks/plan050-openclaw-hud-event-automation/`.
