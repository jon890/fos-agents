...
| ADR-015 | career-os ADR을 개별 파일로 관리하는 파일럿 예외 | Accepted | career-os만 `docs/adr/` 개별 파일과 `INDEX.md` 구조를 사용하고, 다른 워크스페이스는 `docs/adr.md` 단일 누적 유지 |
| ADR-016 | common-pitfalls file-per-pattern 구조 | Accepted | 단일 `common-pitfalls.md`를 `INDEX.md` router와 항목별 pattern file로 분리해 필요한 함정만 읽는다 |
...
## ADR-016 — common-pitfalls file-per-pattern 구조
