...
| `accountbook/` | [`accountbook/AGENTS.md`](../accountbook/AGENTS.md) | 금융 스크린샷 기반 가계부 입력 자동화 |
...
Wrangler가 반환한 배포 고유 주소와 분기 별칭을 검증한다.
분기 별칭이 실제 HTTP 검증을 통과하면 기본 공유 링크로 사용하고, 그렇지 않으면 검증된 배포 고유 주소를 사용한다.
