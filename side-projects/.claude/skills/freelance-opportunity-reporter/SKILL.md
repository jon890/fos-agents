...
6. 리포트를 작성하고 게시한다.
   ...
   필수 플랫폼의 인증 만료나 접근 제한은 알릴 내용이 없는 상태가 아니다.
   다른 플랫폼에서도 후보를 찾지 못했더라도 `[SILENT]`로 끝내지 말고,
   확인한 플랫폼과 제외 사유를 짧게 알린다.
...
- 위시켓 후보가 로그인 상세에서 미팅·현장 작업·검수 조건까지 확인된 공고인지
- `remote_only_pass`가 `true`가 아닌 공고가 사용자용 표에 없는지
